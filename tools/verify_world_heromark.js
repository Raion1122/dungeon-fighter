...
/* ⭐ #45 (2026-09-03): 街道の出来事が `roadEvent` を **1 個だけ**足した (world.html:1295)。
   依頼書 `実装依頼書/2026-09-03_road-events.md` §8 (4b) が「#45 が足すのは読むだけの窓 1 個」と
   最初から宣言しているので、これは **正当な期待値更新** (⛔ 弱めではない —— 下の assert 本体は
   1 バイトも触っていない = 消えたキーが 0 件 / 増えたのはこの表の分だけ、を今も同じ厳しさで見る)。
   ⛔ ここへ「増えるかもしれないキー」を先回りで足さない。実際に足したときだけ書く。 */
const NEW_SEAM_KEYS = ['heroMarkGeom', 'heroMarkOn', 'roadEvent'];   /* sort 済み */
/* ── §4 の計測パラメタ (項目 4) ──────────────────────────────────────────────
...
const ASSERTS = [
  ...
  ['3a', 'Object.keys(window.__world) が **着手前 HEAD (' + BASE_REV + ') のキー集合**を'
    + 'すべて含み、増えたのは ' + NEW_SEAM_KEYS.join(' / ') + ' の '
    + NEW_SEAM_KEYS.length + ' つだけ'
