...
const MUTATIONS = {
  ...
  noopen: [
      ...
      '      /* ★変異noopen: 開けない */'],
  usedirsvia: [
      '        for (const ex of ((node && node.exits) || [])) if (ex.dir) dirs.add(ex.dir);',
      '        for (const dd of nodeExitDirs(node)) dirs.add(dd);   /* ★変異usedirsvia */'],
};
...
(async () => {
  ...
  mark('§6 撤退スイッチ / dev ゲート / 不変性');
  {
    ...
    const warns = [];
    const p1 = await bootPage(browser, base + '/index.html?doors=states', STAGE, errs, { warns: warns });
    ...
    const p3 = await bootPage(browser, base + '/index.html?diag=1&intel=0', STAGE, errs);
