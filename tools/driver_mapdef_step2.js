 ...
 *    ここを広げるときは §7 の HARD_MS と 4 分上限の両方を必ず見直すこと。 */
const CUSTOM_ROOMS = [
...
(async () => {
  ...
  try {
    ...
    if (SKIP_AUTOPLAY) {
       ...
       * ⚠⚠ HARD_MS は **index.html 自身のラン ハード上限 4 分より必ず短く**する。
       *   超えるとゲーム側が console.error("[DIAG][run-timeout] …") を出して強制終了し、
       *   7e (pageerror 0 件) が「ドライバが長く待ちすぎたせい」で落ちる = 原因が読めなくなる。 */
