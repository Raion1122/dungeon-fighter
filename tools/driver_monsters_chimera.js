...
(async () => {
  ...
  const profile = require('./_pptr_profile')('df_chim_');
  ...
  for (let attempt = 0; attempt < 6 && !(breath.fired && breath.maxHit >= 2); attempt++) {
    ...
    await pageB.goto('http://localhost:' + PORT + '/index.html?autoplay=30&magesleep=0', { waitUntil: 'domcontentloaded', timeout: 30000 });
