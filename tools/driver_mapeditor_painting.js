...
(async () => {
  ...
  try {
    ...
    let CATALOG = { srcs: [] };
    {
       ...
       *   railKit = つながる線路 は STEP 2.5 で追加)。 */
      check('§1 1i sceneryKinds() が 12 種 (grass/reed/log/detail/rubble/cart/rail + pillar/chair/table/wreck + railKit)',
    ...
    mark('§6 退化 — カタログ取得失敗を UI と console.warn の両方で知らせる');
    {
      ...
      check('§6 6a 成功時の #paintNote が「1枚絵 12 枚 / 情景 12 種」(.ng なし)',
        /1枚絵 12 枚/.test(D.ok.note) && /情景 12 種/.test(D.ok.note) && D.ok.noteNg === false, D.ok.note);
