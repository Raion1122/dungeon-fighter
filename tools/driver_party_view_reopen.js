 ...
 *   文言が出発を指していないこと (1d) は、同じ 1 つの主張の両面。
 *   ⚠ #60 (2026-09-08): review の行き先の**名前**は「準備画面」→「酒場」へ変わった
 *     (準備画面は #55 で廃止済 = 行き先が無かった)。⇒ (1d)(4c) は行き先の名前を縛らない形へ
 *     言い直してある。⛔ 新しい名前を写経し直さないこと。
 ...
 *     N1 → (1d)(4c) ⭐#60 で (4c) が加わった   N2 → (1b)(1c)(3b)
 *     N3 → (1e)(3b)                   N4 → (2a)
 *     N5 → (1b)(1c)(1d)(1e)(3b)
...
function withAutoParty(p) {
  /* ⚠ #55: withPrepScreen を通した **後** の文字列で '?' を見ること。
     元の p を見ると '?prepskip=0?recruittalk=0' という壊れた URL になる。 */
  const q = withPrepScreen(p);
  return q + (q.indexOf('?') >= 0 ? '&' : '?') + 'recruittalk=0';
}
async function openClean(browser, urlPath) {
  ...
  await page.goto('http://localhost:' + PORT + withAutoParty(urlPath), { waitUntil: 'domcontentloaded', timeout: 45000 });
  await sleep(900);
  return page;
}
...
async function tapOverlay(page) {
  await page.evaluate(() => {
    /* #35: 出発の口が #pmDepart へ移った (背景タップでは閉じなくなった)。
       ...
       ⚠ review モードではラベルが差し替わる (行き先が出発ではない) が、id は同じなので掴み方は変わらない。
...
(async () => {
  ...
  try {
    ...
    /* ⚠ #35 STEP3 で【測定点を移した】。文言そのものを verbatim で縛っていたが、背景タップで
       ...
       ⚠⚠ #60 (2026-09-08) で **もう一度**測定点を移した。行き先の名前が「準備画面」から
         「酒場」へ変わった —— その準備画面は #55 で廃止済で、review の文言だけが
         行き先の無い嘘の導線として残っていたため。
         ⛔ 新しい行き先の名前を写経すると、次に行き先が動いた日にまた腐る。
           ⇒ 縛るのは ① **出発とは言わない** ② **待機フェーズの文言である (空文字ではない)**
             の 2 つだけ。行き先の**名前は縛らない**。
       ⭐ N1 (review でも出発側の文言) は ① で赤。N5 (review を渡さない) は
         review が丸ごと死んで待機フェーズへ入らないので ② で赤 (どちらも実走で確認)。 */
    check('(1d) ★受入条件: 待ち文言が「出発」とは言わない待機文言である (閉じた先は出発ではない)',
      o1.hint.length > 0 && o1.hint.indexOf('出発') < 0 && o1.hintWait === true,
      '実際 = "' + o1.hint + '" (pmWait=' + o1.hintWait + ')');
    ...
    /* ⚠ #35 STEP3 で【測定点を移した】(1d と同じ理由)。verbatim の一致から
       ...
       ⚠⚠⚠ #60 (2026-09-08): 「review の文言が漏れていない」を **旧ラベルの逐語**で持っていたため、
         ラベルが変わった瞬間にこの条件が **恒真 = 空振り**になった (素は緑のままなので気づけない)。
         ⇒ 逐語をやめ、**§1 で実測した review の待ち文言そのもの (o1.hint)** と比べる。
         ⭐ これなら行き先の名前が何度変わっても腐らないし、N1 (review でも出発側の文言) で
           両者が同一になって赤くなる = 検出力はむしろ増えている。 */
    check('(4c) 全開示後は文言が開示中から切り替わり、出発の導線を語る (review の文言が漏れていない)',
      n1b.hint !== n1a.hint && n1b.hint.indexOf('出発') >= 0
      && n1b.hint !== o1.hint && n1b.hint.indexOf('カード') >= 0
      && n1b.nFilled === n1b.nCols,
      '"' + n1a.hint + '" -> "' + n1b.hint + '" / ' + n1b.nFilled + '/' + n1b.nCols
      + ' / review の待ち文言 = "' + o1.hint + '"');
    await page2.close();
