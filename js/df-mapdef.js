...
(function (global) {
  ...
  var LINT_PAINTING_ASPECTS = [
    ...
    /* ★[#11 2026-08-22] codex1 納品の卓上バトルマップをボスノードへ丸ごと貼る大部屋
     *   (bandits-forest/n7 = 52×26)。
     * ⚠ この一覧は **カタログが引けないときだけ**の退避判定。lintRun は
     *   ROOM_PAINTINGS_DEF (index.html) より前に走るので、起動時だけここへ落ちる。
     *   カタログが登録されていれば paintingAspectFits は**指定したその絵の tileBounds**と
     *   比べる (P7 で直した強い経路) ので、一覧の粗さはそちらでは効かない。
    ...
    /* ★[#58 2026-09-07] 同じ理由で **ボスノード**の大部屋 (lizard-swamp/n7 = 29×20)。
     * ⚠⚠ 依頼書 #58 §3 は「js/df-mapdef.js は触らない」と書いていたが、#53 と同 #58 の n6 と
     *   まったく同じ形で **実測で崩れた**: 起動時 (カタログ登録より前) の lintRun がこの一覧へ
     *   落ちるので、足さないと 29×20 が毎回 graph-painting-aspect の warning を出し、
     *   driver_graph_p6 (2c-lizard-swamp) が赤くなる。
     * ⛔ 触ったのはこの在庫 1 行だけで、resolve() の規則には手を入れていない。 */
    { w: 29, h: 20, label: "ノード大部屋 29×20 (29:20)" },
    /* ★[#66 2026-09-11] 同じ理由で **神殿の大部屋 2 枚** (undead-temple/n4 = 28×18 /
     *   n7 = 20×13)。⚠⚠ 依頼書 #66 §3 の変更範囲は js/df-mapdef.js を**挙げていなかった**が、
     *   #11 (52×26) / #53 (30×21) / #58 (34×22・29×20) とまったく同じ形で **実測で崩れた**:
     *   起動時 (カタログ登録より前) の lintRun はこの一覧へ落ちるので、足さないと
     *   28×18 と 20×13 が毎回 graph-painting-aspect の warning を出し、
     *   driver_graph_p6 (2c-undead-temple) が赤くなる。
     * ⭐ 砦 #63 が足さずに済んだのは偶然 — 30×20 は **3:2** で既存の「ノードボス 9×6」と
     *   比が一致したため (この一覧は**サイズではなく比**で当てる)。神殿の 14:9 と 20:13 は
     *   既存 8 種のどれとも一致しない。
     * ⛔ 触ったのはこの在庫 2 行だけで、resolve() の規則にも paintingAspectFits の
     *   判定式にも手を入れていない。 */
    { w: 28, h: 18, label: "ノード大部屋 28×18 (14:9)" },
    { w: 20, h: 13, label: "ノード大部屋 20×13 (20:13)" },
    /* ★[#67 2026-09-16] 同じ理由で **竜の巣の大部屋 2 枚** (dragon-lair/n4 = 31×20 /
     *   n7 = 30×19)。#11 (52×26) / #53 (30×21) / #58 (34×22・29×20) / #66 (28×18・20×13) と
     *   まったく同じ形で、足さないと起動時 (カタログ登録より前) の lintRun がこの一覧へ
     *   落ちて 31×20 と 30×19 が毎回 graph-painting-aspect の warning を出し、
     *   driver_graph_p6 (2c-dragon-lair) が赤くなる (5 例目)。
     * ⚠ 値は**焼き上がりの実測**。1488x960 / tile 48 = 31×20、1440x912 / tile 48 = 30×19。
     * ⭐ どちらも既存 10 種のどれとも比が一致しない
     *   (31×20 = 1.5500 vs 最も近い 28×18 = 1.5556 → 31*18=558 ≠ 20*28=560 /
     *    30×19 = 1.5789 vs 20×13 = 1.5385 → 30*13=390 ≠ 19*20=380)。
     *   ⭐ 砦 #63 が足さずに済んだのは偶然 — 30×20 は 3:2 で「ノードボス 9×6」と同比だった。
     * ⛔ 触ったのはこの在庫 2 行だけで、resolve() の規則にも paintingAspectFits の
