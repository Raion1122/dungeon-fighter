...
⚠ 起草時点では clean だが、**着手時には #47 の 4 コミットが入っている前提**。
着手前に `git status` と `git log --oneline -6` を見て、#47 が着地済みであることを確認する。
...
| ファイル | 変更 |
|---|---|
| `tools/verify_player_sheet.js` | 導出ヘルパ 1 本の新設 / assert **(0e)** の新設 / **(2c)(2d)(8a)(8f)** の期待を導出へ / 変異 **2 本**の追加 / `SECTIONS` と `MUT_ORDER` への登録 |
