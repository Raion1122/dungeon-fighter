...
- **起草**: 2026-08-29(計画窓 `claude-36`) / **ステータス**: ✅ **完了 `c6862a9`+`f112f53`+`edd42d0`+`b7c70ef`+`1f518d2`**(2026-08-30〜31 / dev-loop 4 項目・停止 0 回)
  — 実装結果は **§13**。✅ **push 済み `097344a`**(2026-08-30)。
...
|---|---|---|
| 仲間の死 | **今回は死なせない**。倒れても帰還すれば名簿に残る | ⛔ 恒久ロスト / ⛔ 重傷(数クエスト雇えない)。**シナリオ2 のクリア率が実測 0/3 のまま恒久ロストを入れると詰む**(会議でノエルが指摘)。死は **候補③「冒険の賭け金」**で寺院・蘇生費用と一緒に入れる |
...
- ⛔ `alive` / `dead` は持たない(今回は死なせない)。**後続チケット(候補③「冒険の賭け金」)が足す**。
...
6. 敗北しても名簿が減らないことが「ぬるい」と感じないか
   (⭐ 感じるなら **候補③「冒険の賭け金」** の設計材料になる。ここでは直さない)。
...
- ⛔ **仲間の死・恒久ロスト・負傷の持ち越し**(§1 の決定)。**候補③「冒険の賭け金」**。
