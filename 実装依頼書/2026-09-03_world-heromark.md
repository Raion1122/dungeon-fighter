...
- **起草**: 2026-09-03(計画窓) / **ステータス**: **完了**(2026-09-03)
  ...
  ✅ **実機体感もユーザー確認済み(2026-09-03「ひとまずオッケー」)= 残タスク 0。**
  ⚠ ただし「ひとまず」= 暫定の合格。**▽ を一様に縮めると 18/18 緑のまま通る**という
  **宣言された穴**が残っている(塞ぎ方と、打ってよい条件は **§12-8**)。
...
### 12-7. 残った宿題 → ✅ **2026-09-03 にユーザー確認済み・残 0**

**ユーザー判断(2026-09-03)**: 「実機体験はひとまず、オッケーです。」
⇒ **§9 の 6 項目はすべて受け入れ。寸法・色・高さの変更は 1 件も発生せず**、
`HM_GAP` / `--hm-w` / `--hm-h` / `HM_W` / `HM_H` / `#ffd24a` は**実装時の値のまま確定**。

⚠ ただし「**ひとまず**」= 暫定の合格であって恒久の凍結ではない。
⭐⭐⭐ だから絶対量の assert は**まだ打っていない**(§12-8 参照)。

以下が確認された 6 項目(記録。ローカルは **http 起動が必須**):
...
### 12-8. ⭐⭐⭐ 宣言して残した穴 ——「▽ を一様に縮める」は 18/18 緑のまま通る

隣窓 `claude-be`(#44 敵の名前札)との相互チェックで確定した事実:

**CSS の `--hm-w` / `--hm-h` と JS の `HM_W` / `HM_H` を同時に縮めると、
`verify_world_heromark` は 18/18 緑のまま ▽ だけが小さくなる。**

- (0b) は「実描画の矩形 ÷ zoom」と「JS の `HM_W*2` / `HM_H`」の突き合わせ = **相対比較**。
  両方を同率で動かすと差は 0 のまま。
- (1a)(1b) も主人公の矩形が基準なので同様。
- ⭐ 隣窓が同型の穴を 2 本見つけている: `driver_grid_p7` の (4c)(4f)(「ズーム時 vs 素」の相対比較)、
  `driver_heromark_signplate` の `labelTop`(上端は `placeUnscaledUi` の `dy` が決めるので札が縮んでも不変)。
  ⇒ **相対比較の assert は「両方が同じだけ壊れる変更」を原理的に検出できない。**

⭐⭐⭐ **これは「宣言された穴」であって「気づいていない穴」ではない。**
§8「⛔ 測らないこと」が「▽ の見かけの大きさは実機で見て調整する余地を残す」と明記しており、
(0b) が縛っているのは大きさではなく **「CSS と JS が食い違っていないこと」**だけ。

**塞ぐなら**(判断基準つき):

- ⭐⭐⭐ **assert は決定を記録するものであって、決定を先取りするものではない。**
  絶対量の assert を打ってよいかの判定基準は **「その値はもう決まっているか?」**。
- 2026-09-03 のユーザー判断は「**ひとまず**オッケー」= **暫定の合格**。
  恒久の凍結ではないので、**上限を打つのは時期尚早**(まだ大きくする可能性がある)。
- ⭐ 打つなら **下限だけ**。源は ▽ と独立していること:
  **`▽ の高さ ÷ 拠点の点(.worldNode-site .worldNodeBody = 17px)`** か
  **`▽ の高さ ÷ .worldSign の高さ`**。どちらも ▽ の CSS/JS とは別の出所なので、
  「両方を同率で縮める」を落とせる。⛔ 上限は書かない(§9-4 の伸ばす余地を殺すため)。
- 隣窓 #44 の (2f) が**今すぐ打てた**のは、閾値 0.30 が「まだ決まっていない値」ではなく
  **既に決まっている値(ユーザーが『7 割』と言った)から逆算した歯止め**だったから。同じ規則の両側。
