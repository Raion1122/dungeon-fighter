...
- **起草**: 2026-09-16(起草窓 `claude-c5` / セッション `52dfe631`) / **ステータス**: **完了**(2026-09-17 着手 → 2026-09-18 03:50 着地。実装窓 `b0fa1865`・dev-loop 5 項目。⛔ **未 push**)
  ← **承認済**(2026-09-16 ユーザー承認)。着地の一覧・受入の最終結果・崩れた点 25 件は **§12-5 総括**
...
- **測定日**: 2026-09-17 22:58 → 2026-09-18 03:48(実装窓 `b0fa1865`・dev-loop 項目5)/ HEAD = `d9c8cbf`(項目4)・着手時の作業ツリー clean。⛔ push していない。
- 触ったのは **本書と `実装依頼書/README.md` だけ**。本番(`index.html` / `tavern.html` / `audio.js` / `js/*`)も `tools/` も **0 バイト**
  (走査の前後で `git hash-object index.html tavern.html audio.js` と `tools` / `js` のツリー hash が一致 = `blob_before5.txt` = `blob_after5.txt`)。
- ⚠ 終了時の `git status --short` は**起草窓の未追跡 `実装依頼書/2026-09-18_spell-off-visibility.md` の 1 行だけ**(こちらは 1 バイトも触っていない。README の #69 行も無傷)。

#### (a) 走らせたもの・所要・再起動待ちの兆候

| 走行 | 内訳 | 所要 |
|---|---|---|
| 本走査 `after68` | 項目1 の `runlist68b.txt` **144 走行を同じ順**で + 末尾に 3(`verify_bolt_aim` / 同 `--negative` / `driver_action_priority --negative`)= **147 走行** | **251.7 分**(2026-09-17 23:00:41 → 2026-09-18 03:12:22。1 本ごとの秒の合計は 249.7 分) |
| `probe_party_size` | 基準と同じ 600 秒の打ち切り(名簿の外・単独) | 10.0 分(03:12:22 → 03:22:22) |
| 再走 `rr_i5` | 色が動いた 5 本 × 3 回 = **15 走行**(直列・交互) | 16.1 分 |
| `98419b0` の腕 | `driver_monsters_chimera` / `driver_monsters_griffon` × 3 回 = **6 走行** | 6.3 分 |

- ⛔ `timeout` で包んでいない。⛔ 並列にしていない(1 本ずつ `subprocess.run`)。
  走行の直前ごとの並走検査は **147/147 が `procs=0 ports=`**(`foreign_after68.txt` は 1 度も生成されなかった)。再走 15 本・`98419b0` の 6 本の前後も同じ。
- **再起動待ちの兆候**(着手前 22:59:14 / 後始末後 03:47:58 の 2 回測定・`pre_state5.ps1`):
  `WindowsUpdate\Auto Update\RebootRequired` **なし** / `Component Based Servicing\RebootPending` **なし** / `PackagesPending` なし / `PostRebootReporting` なし /
  `PendingFileRenameOperations` は 20 件あるが**全部プリンタドライバの差し替え**(`spool\drivers\x64\3\...`)で WU の再起動要求ではない。
  `LastBootUpTime` は前後とも **2026-09-17 00:30:46**(前夜の WU 再起動のまま)⇒ **今回の走査は途中で切れていない**
  (前夜の症状 = exit `0x40010004` の後に `0xC000026B` が 0 秒で並ぶ、は 1 件も出ていない。147 走行すべてに START と DONE が揃っている)。
  ⚠ ただしアクティブ時間は **6:00〜0:00** なので、走査が深夜にまたがる限りこの危険は消えていない。
- **LISTEN(8000〜11000)**: 着手前も後始末後も `127.0.0.1:9010`(lghub_agent pid 19412)/ `127.0.0.1:9180`(lghub_updater pid 4764)の **2 件だけ**・chrome 0 本・node は MCP サーバの 24 本だけ。
  落としたプロセスは `probe_party_size` の打ち切り(ツリーごと `taskkill /F /T`・残存 node「なし」)だけで、着手前から居たものには触っていない。

#### (b) 2 経路の突き合わせ

`py fp68_i5.py compare ..\baseline68\result_base68b.FINAL.tsv result_after68.FINAL.tsv base68b_vs_after68`(→ `compare68_base68b_vs_after68.txt`)。共通 **144 本** / B だけ 3 本(新しい 3 走行)。

| 経路 | 結果 |
|---|---|
| exit | 緑→赤 **4**(`driver_monsters_chimera` / `driver_monsters_griffon` / `driver_monsters_hobgoblin` / `driver_speech_engine`)/ 赤→緑 **1**(`driver_monsters_umberhulk`)/ 非緑のまま **13** |
| ① assert id の集合 | 新しい FAIL id **5**(chimera `4` / griffon `3`,`4` / hobgoblin `e` / speech_engine `4`)/ B で消えた id **0** / B で増えた id **0** |
| ② 判定行の多重集合 | 新しい FAIL 行 **7**(上の 4 本だけ)/ PASS から消えて FAIL にもなっていない行 **5** / 増えた PASS 行 5 |

- ⇒ **動いたのは上の 5 本だけで、残り 139 本は判定行の多重集合が完全一致**。「本数が同じで中身が入れ替わる退行」も②で否定できた。
- ②の「消えた PASS 5」は §12-0 (g) と同じ**鍵に実測値が混ざる表記ゆれ**で退行ではない:
  `driver_grid_p5` の `(2b1)` 系 4 行が `ally:elf` ↔ `ally:rogue`(乱数で組まれた職業名が鍵に入る)/ `probe_s2_clear` の `残敵=#` ↔ `残敵=##`(桁数)。
- 判定行の総数: 基準 PASS **6712** / FAIL **36** → 今回 PASS **6908** / FAIL **70**。
  増えた FAIL 34 = `verify_bolt_aim --negative` の**変異の走行が出す赤 28**(設計どおり)+ フレーク 7 − umberhulk 1。

#### (c) 色が動いた 5 本の単独再走 — ⛔ どれも #68 由来ではない

⚠ 5 本とも **観測窓の空振り**(`entries=0` / レンジ 0)型で、#68 が触った直線呪文とは無関係。

| 本 | 判定行 | HEAD(`d9c8cbf`)の赤/標本 | `98419b0`(#68 の前)の赤/標本 | 判定 |
|---|---|---|---|---|
| `driver_monsters_chimera` | `(4) 回帰: 非flight ミノタウロス は既定` — `front=null rear=null (entries=0)` | **1/4**(本走査 赤・再走 3 本とも緑) | **2/3**(`altdrv` で実走。**同じ判定行・同じ `entries=0`**) | フレーク(新顔) |
| `driver_monsters_griffon` | `(3) swoop … grifRearMid=0/N` ほか | **3/4**(本走査 赤・再走 緑/赤/赤) | **4/7**(基準 4 標本で 1 + `altdrv` 3/3。同じ `(3)` 行) | 既知フレーク |
| `driver_monsters_hobgoblin` | `(e) 孤立配置で… acProbe 記録あり` ほか 1 行 | **1/4**(本走査 赤・再走 3 本とも緑) | **3/4**(§12-0 (e)(b) の観測。同じ 2 行) | 既知フレーク |
| `driver_speech_engine` | `(4) カメラが実際に動いた` — `camXレンジ=0.5px` | **1/4** | **1/4**(§12-0 (e)(b)。同じ行・同じ症状) | 既知フレーク |
| `driver_monsters_umberhulk` | `(3) 再発火: 同一 enemyIdx が2回以上 gaze` | **2/4**(本走査 緑・再走 赤/赤/緑) | **4/4**(§12-0 (e)(a) で「安定した赤」と分類していた) | ⭐ **赤→緑はフレーク** |

- ⭐⭐⭐ **`98419b0` の腕は「配信の 2 ファイルだけ差し替える」形で採った**(`make_alt_i5.py`):
  `git cat-file --filters 98419b0:index.html` で**チェックアウトと同じ行末変換を通して**書き出し(CRLF 39464 = LF 39464)、
  ドライバの写しは **ROOT をリポジトリに固定したまま `/index.html` と `/tavern.html` だけ古い方から配る**(assets / js / audio.js はリポジトリのまま)。
  ⚠ 同じ方法で `HEAD` を書き出すと**作業ツリーとバイト一致**することを先に確かめてある(⛔ `git show` は blob をそのまま出すので LF になる = 項目3 の `root98419b0\` はそうなっていた。`git worktree` の CRLF 化けと同型の罠)。
- **構造の証明(実走に依らない 2 経路)**: この 5 本はどれも **LB を 1 度も撃たない**。
  ① `DEFAULT_KNOWN.mage`(`index.html:13513`)は `magic-missile` / `fire-bolt` / `arcane-shield` だけで `lightning-bolt` を含まない。
  ② `initAllySpellSlots` の**完全ゲート** `isSpellKnown(classKey, id)`(`:13940`)が未習得の呪文を配分から外す ⇒ `ally.equippedSkills` に LB が入らない ⇒
  `mageAI` の `ally.equippedSkills.includes("lightning-bolt")`(`:30130`)が偽 ⇒ `allyLightningBolt` に到達しない。
  ⇒ **#68 の差分はこの 5 本の実行経路に存在しない**(項目3 (a) の「実プレイで偶然 LB が出るようになる本は増えない」の裏取り)。

#### (d) 非緑の集合 — 母集団側は着手前と同じ

| 区分 | 着手前(`base68b`) | 今回(`after68`) |
|---|---|---|
| 安定した赤 | 11 本 | **10 本**(`driver_monsters_umberhulk` が緑に振れた。⭐ 安定ではなかった = (f)1) |
| 引数なしの使い方ガード(赤ではない) | 3 本(`probe_bandit_map` / `probe_s2_fold` / `probe_swamp_map`。引数腕は 3 本とも exit 0) | **3 本**(同じ。引数腕も 3 本とも exit 0) |
| フレークが赤側に出たもの | 0 本 | **4 本**((c)) |
| 名簿の外 | — | `driver_action_priority --negative` = **着手前から exit 1**((e)) |
| 合計(非緑) | 14 | **18**(= 13 + 4 + 1) |

- `probe_n4_stall` は今回も **exit 0**(「停滞を捕捉」)。§12-0 (e)(c) のとおり**どちらの色も合否に数えていない**。
- `driver_field_step6` は今回も `55/59`(`(C-bandits-forest)` / `(C-lizard-swamp)` / `(C-orc-fort)` / `(C-undead-temple)` の 4 本が赤)= 着手前と同じ集合。
- `probe_party_size` の 600 秒打ち切りは、判定行の列が基準と**数字を除いて同一**(`NG (1e)` / `NG (2a)` / `NG (2b)` / `NG (2z1)`〜`(2z4)` / `OK (2z5)` / `OK (4z0) hits=1`)。残存 node は「なし」。

#### (e) 新しい 3 走行

| 走行 | exit | 集計行 | 秒 |
|---|---|---|---|
| `node tools/verify_bolt_aim.js` | **0** | **`23/23 PASSED   FAILED 0   PENDING 0`** | **2233.4**(37.2 分) |
| `node tools/verify_bolt_aim.js --negative` | **0** | `--negative OK: 10 本すべて担当ラベルが赤くなりました (空振り 0)` | 82.7 |
| `node tools/driver_action_priority.js --negative` | **1** | `[driver] 負のコントロール N3 の注入点が 2 箇所 (期待 1)。アンカーが腐っています:` / `         const equippedIds = apEquippedIdsFor(slot, classKey);` | 0.1 |

- `verify_bolt_aim` の **3 回目の完走**(項目4 の C / D に続く E)。実プレイ 24 走行・pageerror 合計 0。主要な値:
  - **(2a)** マップ平均 素 **85.0%** / 撤退 24.6% / 差 **+60.4pt** / 素の cast 合計 **33**。
    マップ別 = 廃坑 素 21/30 = **70.0%**・撤退 13/40 = 32.5% / 森 素 12/12 = **100%**・撤退 3/18 = 16.7%。
    ⚠⚠ **廃坑の素がちょうど 70.0% = 閾値ぴったり**(C 61.0 / D 68.3 / E 70.0%)。判定はマップ平均なので通るが、**廃坑単独は 3 回とも 70% の線上か下**。
  - `[record]` 合算 pooled 素 33/42 = 78.6% / 撤退 16/58 = 27.6% / 差 51.0pt。
  - **(0c)** 素の attempts 合計 **42**(廃坑 9,5,2,5,9 / 森 4,2,2,2,2。各走行 ≥ 1)。
  - **(0d)** 旧仕様の再計算と **58/58 一致** / **(2b)** 新仕様の再計算と **54/54 = 100%** / **(4b)** 新仕様との食い違い **31**。
  - `[record] (2c)` 混成の CC attempts 合計 **3**・**0 の走行 3/4**(⚠ 居座りの実害が一部残っている観測の 3 例目)。
  - `--negative` の担当と赤の集合は項目4 の (d) の表と**完全に同じ**(`reach3` → (1a)(1b)(1c2)(1e)(1f)(1g)(1h)(5a) … `flavor3` → (3a))。
- `driver_action_priority --negative` は **#35 `97f350d`(2026-08-29)以来の N3 アンカー腐敗**で、**文面まで項目3 の実測と 1 文字も違わない**
  (項目3 は `98419b0` の本番でも同じ exit 1 を実測済み)⇒ ⛔ 緑→赤に数えない。別チケット候補 (iv)。

#### (f) 崩れた主張(項目5)

| # | 主張 | 実測 |
|---|---|---|
| 1 | §12-0 (e)(a)「安定した赤 11 本」 | **`driver_monsters_umberhulk` は安定ではない**。本走査で緑・再走 2/3 赤。4 標本で同じ色が続いても「安定」とは言えない(#67 の教訓の 2 例目) |
| 2 | §12-0 (k)「フレークとして単独再走してから判定する本」の一覧 | **`driver_monsters_chimera` が入っていなかった**(新顔の 2 例目)。基準の 2 標本はどちらも `entries=1` の**ぎりぎりの緑**で、フレークの予兆は「緑」の中にあった |
| 3 | (項目1 の道具の前提)1 タグ = 1 本 1 ログ | ⚠ `sweep68.py` は**同じ本を複数回走らせるとログ名が衝突して上書きされる**(再走 15 本で最後の 1 回しか残らない)。基準の名簿が 1 本 1 回だったので露呈していなかった。⇒ 再走の判定行は `logs\alt\<本>_<rev>_<n>.log` のように別名で残した |
| 4 | 指示「緑→赤が出たら #68 由来か切り分ける」 | 切り分けは**再走だけでは足りなかった**(chimera は HEAD 再走 3/3 緑 = 「たまたま」に見える)。`98419b0` の腕で**同じ判定行の赤**を出して初めて決着した |

⭐ 当たったもの: LISTEN は前後とも lghub の 2 件 / `verify_bolt_aim` の所要は約 37 分 / `driver_action_priority --negative` は同じ文面の exit 1 / `probe_party_size` の打ち切りの中身 / **#68 由来の緑→赤 0 本**。

#### (g) 生ログ・スクリプト

`C:\Users\PC_User\AppData\Local\Temp\claude\c--Users-PC-User-Desktop------------\b0fa1865-6afd-4eba-a0b1-818df2247585\scratchpad\item5\`

- 名簿 = `runlist_i5.txt`(147 行。先頭 144 行は `baseline68\runlist68b.txt` と完全一致)/ 再走 = `runlist_rr_i5.txt`(15 行)
- 結果(凍結)= **`result_after68.FINAL.tsv`** / `result_rr_i5.FINAL.tsv` / 進捗 = `progress_after68.txt` / `progress_rr_i5.txt`
- 生ログ = `logs\after68\`(147 本)/ `logs\rr_i5\`(⚠ 同名で上書き = (f)3)/ `logs\alt\<本>_98419b0_<n>.log`(6 本)/ `logs\party_size\`
- 指紋 = `fp68_ids_after68.tsv`(①)/ `fp68_lines_after68.tsv`(②)/ `fp68_after68.json` / 要約 = `summary68_after68.tsv`
- 比較 = **`compare68_base68b_vs_after68.txt`** / 走査器 = `sweep68_i5.py` / 指紋器 = `fp68_i5.py`(どちらも項目1 の写しで出力先だけ item5)
- `98419b0` の腕 = `make_alt_i5.py` → `root98419b0\index.html` / `tavern.html` + `altdrv\<本>_alt.js`
- 状態の控え = `pre_state5.ps1` → `pre_state5_before.txt` / `pre_state5_after.txt` / `blob_before5.txt` / `blob_after5.txt`
- 打ち切り走行 = `run_party_size_i5.py` → `probe_party_size_note.txt`
...
| 項目 | コミット | 中身 |
|---|---|---|
| 1 | `a23e002` | 着手前の基準取り(母集団 133 本 / 名簿 144 走行 / 非緑 14 本の 3 分類)§12-0 |
| 2 | `e2927c3` | 本体 — 敵へ向けて引く直線(撤退 `?boltaim=0` / 長さは `getRange` から導出 / 壁で止める)§12-1 |
| 3 | `e42b7ea` | 既存 golden の言い直し = **不要**(腐りうる本は 4 経路の union で段1 の 4 本・緑→赤 0)§12-2 |
| 4 | `d9c8cbf` | 受入 `tools/verify_bolt_aim.js`(新規・base 10301 / 変異 10302〜10311)§12-3 |
| 5 | 本行を書いたコミット | 母集団 147 走行の非退行 + §12 総括 + 台帳 #68 行 §12-4 |

⚠ 途中に起草窓のコミット `4ef76eb`(#69 起草)が挟まっている。本番・`tools/` の差分は 0 バイトなので測った木は同じ。⛔ **どの項目も push していない**。

#### (b) 受入の最終結果

| 走行 | 結果 |
|---|---|
| `verify_bolt_aim`(素) | **23/23 PASSED / FAILED 0 / PENDING 0** を **3 回**(項目4 の C `2232.5s` / D `2232.7s`、項目5 の E `2233.4s`) |
| `verify_bolt_aim --negative` | **10/10 空振り 0** を **3 回**(担当と赤の集合はすべて同じ) |
| 既存の番人 | `verify_aoe_coverage` 28/28 + `--negative` 10/10(m8 = (1c)(6b) 赤)/ `verify_cone_cast` 19/19 + `--negative` 41/41 / `driver_field_step7` 79/79 / `driver_action_priority` 92/92 — すべて基準どおり |
| 母集団の非退行 | 147 走行・251.7 分。**2 経路とも #68 由来の緑→赤 0 本 / 新しい FAIL 行 0 件**(動いた 5 本はすべて `98419b0` でも同じ色が出る観測窓のフレーク) |

- **撤退スイッチ = `?boltaim=0`**。判定は `index.html` の `BOLT_AIM_ON`(`:28451` の `get("boltaim")`)**1 箇所**だけ。
  付けると探索・ダメージ・無傷表示・描画の終点が 2026-09-16 以前(術者から 3 マス・8 方向・壁を見ない)へ戻る(実プレイ 58/58 で旧仕様の再計算と一致)。
  ⚠ 酒場(`tavern.html`)の flavor は別ページなので `?boltaim=0` でも「10 タイル」のまま = 仕様(§5-5)。

#### (c) 依頼書の主張が崩れた点 — **25 件**(§12-0〜§12-4 から重複を除いて集約)

| # | 主張(出どころ) | 実測 |
|---|---|---|
| 1 | §4-1「基準は `157a9ca`・clean を期待」 | HEAD は `98419b0`、走査は起草窓の `4ef76eb` の上。ただし本番・`tools` の差分は 0 バイト |
| 2 | §2-6「`verify_cone_cast` は変更不要 ⇒ 見なくてよい」 | `--negative` の 3 変異(`retreatdead` / `reachdrift` / `coldstale`)が #68 の触る行を**逐語で掴んでいた** ⇒ 基準に `--negative` 腕を足した |
| 3 | 指示「今回の色は #67 項目6 と一致するはず」 | 共通 139 本のうち 138 本一致。不一致 1 本 = `probe_n4_stall`(同じ木で両方の色) |
| 4 | メモ「`probe_n4_stall` は健全なとき exit 1」 | 片側だけ。exit 0 =「停滞を捕捉」⇒ **どちらの色も合否に使わない** |
| 5 | 指示「非緑はフレーク 5 本が特に」 | 4 標本で赤が出たのは 3 本。**新顔 `driver_monsters_hobgoblin` 3/4 赤** |
| 6 | 指示「段2 は #67 では 134 本」 | `index.html` / `tavern.html` で引き直すと **133 本** |
| 7 | (暗黙)scratchpad は空のはず | 同じ項目1 の**中断した走査**が残っていた(前夜の Windows Update 再起動が 136 本目で切った) |
| 8 | §8 変異表「`nowall` → (1d) が赤」 | (1d) は赤くならない(壁の向こうの敵は視線で候補から落ちる)⇒ 盤面 **(1d2)** を新設 |
| 9 | §8 変異表「`norange` → (1c) が赤」 | (1c) は赤くならない(列の歩きが L で打ち切る)⇒ 盤面 **(1c2)** を新設 |
| 10 | §5-4「既存 `aoeLineSafeAllyTag` は撤退経路が使う」 | 撤退も新しい `aoeTilesSafeAllyTag(best.tiles)` を通した(本体に 3 つ目の「3」を作らないため)。本番の呼び口は 0 件に |
| 11 | §8 (0a)「`boltaim` の判定がちょうど 1 箇所」 | 語 `boltaim` の行は **4**(コメント 3)。判定 `get("boltaim")` が 1 |
| 12 | (暗黙)合成盤面は `verify_aoe_coverage` の作法でそのまま組める | **不発の盤面で CDP が 240 秒ハング**(不発 → `allyBasicAttack` → `allyAdvanceTowardPoint` の rAF が背景タブで止まる)⇒ 即解決にした |
| 13 | (暗黙)lane(最長の床の行)で盤面が組める | goblin-mine には 14×4 の床が無い ⇒ プローブの中で掘った |
| 14 | 指示「`driver_action_priority --negative` が赤なら #68 か切り分ける」 | **#35 `97f350d`(2026-08-29)以来の N3 アンカー腐敗**。変異 11 本が 18 日間 1 度も走っていない |
| 15 | 指示「語 `flavor` は 36 本ヒット」 | コメントを落とすと **32 本**。`MAGE_SKILLS` / 酒場の呪文 UI の flavor を読む本は **0** |
| 16 | §8 名指し「`driver_field_step7` は #68 の番人」 | Part R は LB を装備させて 240 秒実プレイするのに観測キーは箱型 1 件だけ = **#68 の実挙動を実質測っていない**(`(P2)` の line 68.0% はドライバ自前の旧 3 マスの写し) |
| 17 | §8 (0c)「90 秒で attempts ≥ 5(素の各走行)」 | 12 走行中 8 走行が 5 未満(森は 1〜4)⇒ ユーザー決定で「各走行 ≥ 1 + 合計 ≥ 20」へ |
| 18 | §8 (2a)「素の cast 合計 ≥ 20」 | 各 3 回では **21 / 18** と閾値をまたぐ。⭐ **素で撃てると戦闘が早く終わり試行が減る** |
| 19 | §8 (2c)「混成・各 2 回で CC attempts ≥ 1」 | **4 / 0**。決定後の 5 完走 20 走行でも 0 の走行が多い(A 3/4・B 2/4・C 2/4・D 2/4・E 3/4)⇒ assert から外して記録へ |
| 20 | §2-4「上限 98%・視線と交戦中の絞りで下がる ⇒ 70%」 | **マップで二極化**(森 100% / 廃坑 56〜70%)。各 5 回にすると合算は **68.4 / 67.9%** で閾値割れ ⇒ ユーザー決定 E(マップ平均)。⭐ 交戦中の絞りで落ちた試行は **0 件**で、落ちるのは視線と射程 |
| 21 | 項目2 の煙試験「素 71% / 80%」 | 廃坑 1 走行ずつの値。走行別は **33〜86%** |
| 22 | 指示「各変異の走行で (0a)(0b) を起動確認に使う」 | (0a) は `switchdead` で**巻き添えの赤**(判定の行そのものを消す)⇒ 起動確認は (0b) で |
| 23 | §12-0 (e)(a)「安定した赤 11 本」 | `driver_monsters_umberhulk` は**安定ではない**(項目5 で緑・再走 2/3 赤)。4 標本の一致は「安定」の根拠にならない |
| 24 | §12-0 (k)「再走してから判定する本」の一覧 | **`driver_monsters_chimera` が抜けていた**。基準の 2 標本はどちらも `entries=1` のぎりぎりの緑 = 予兆は緑の中にあった |
| 25 | (項目1 の道具の前提)1 タグ = 1 本 1 ログ | `sweep68.py` は**同じ本を複数回走らせるとログが上書き**される ⇒ 再走の判定行は別名で残す必要がある |

#### (d) ユーザー決定(2026-09-17・orchestrator 経由)

1. **(0c) の合算化** — 「素の各走行で attempts ≥ 1、かつ素の全走行の合計 ≥ 20」(元は各走行 ≥ 5)。
2. **LB×8 単独は各マップ 5 回**(廃坑 5 + 森 5)× 素と撤退。(2a) の閾値 3 本は据え置き。
3. **(2c) の記録化** — 混成の CC attempts は assert から外し `[record]` 行だけにする(混成は各 2 回のまま)。
4. **(2a) はマップ平均**(決定 E)— 率も差も**マップごとの率の単純平均**で測る。合算 pooled とマップ別は `[record]` に必ず出す。
   全マップで素・撤退が各 5 走行・attempts ≥ 1 そろわなければ平均せず FAIL。

⛔ 閾値の値(70% / 30pt / cast ≥ 20)はどの段でも変えていない。

#### (e) 別チケット候補

| # | 中身 | 根拠 |
|---|---|---|
| (i) | **不発時に次の呪文へ回す**(枠を減らさず梯子に居座るのをやめる) | (2c) の記録 = CC attempts 0 の走行が決定後の 20 走行の多数。⚠ 廃坑では LB の不発が実際に居座りを作る(森の 0 は梯子の順と 90 秒の長さ) |
| (ii) | **射程外の標的へ LB を選ぶ梯子** | 素の不発の 2 番目の理由が「交戦中の敵が全員 L の外」(C 6 / D 4 / E 2 件。チェビシェフ 11 の例が反復) |
| (iii) | **反射する直線 / 端を狙う視線**(ユーザー提案・起草窓へ申し送り済) | (g-6) の試算 = 反射 1 回で救えるのは約 4 件、端狙いなら視線 29 件中 20 件。⛔ #68 には入れない |
| (iv) | **`driver_action_priority --negative` の N3 アンカー腐敗** | #35 `97f350d` 以来 18 日間、事前検査で止まって**変異 11 本(#19 の N1〜N6 + #34 の N7〜N11)が 1 度も走っていない** |

#### (f) 残り

- ⚠ **§9 の実機・実感の確認は未実施**(ローカルは http 起動が必須。`file://` ではナレ音声が鳴らない)。見るべきは
  ①斜めや半端な位置の敵へ撃つか ②稲妻が狙った敵を通って伸びるか ③壁の手前で止まって見えるか ④コーンオブコールドと併用で両方見られるか。
- ⚠ **廃坑の素の cast 率は単独では 56〜70%**(C 61.0 / D 68.3 / E 70.0%)。案 E の平均は森の 100% と並べた値なので、廃坑だけを見る目は `[record] (2a)` 行に残してある。
- ⚠ 着手前から赤い 10 本 + 引数ガード 3 本 + `verify_codex_map_skill`(母集団外)+ `driver_action_priority --negative` は **#68 の射程外**。`probe_party_size` は 600 秒でも終わらない(#67 から同じ)。
