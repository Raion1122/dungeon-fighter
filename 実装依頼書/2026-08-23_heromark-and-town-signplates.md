...
**ステータス**: **完了 `372321c`**(2026-08-23)。実装結果は §10。
**起草**: 2026-08-23
...
## 10. 実装結果(2026-08-23 実装窓)

**ステータス: 完了。** 受入条件 19 本すべて緑。

- **コミット**: `372321c`(1 コミット。⚠ 街の札と ▽ は同じ `town.html` の中で
  ハンクが混ざるので、ファイル単位 add では分割できない → 1 コミットにまとめた)
- **触ったファイル**: `town.html` / `js/town-map.js` / `index.html` / `tavern.html`(changelog 2 行)
  / **新規** `tools/driver_heromark_signplate.js`
...
| `node tools/driver_heromark_signplate.js --negative` | **4 / 4**(`markhead`→(A3) / `marklow`→(A1)(A5) / `plateflat`→(B1) / `platehide`→(B4) がそれぞれ赤くなった) |
| (C1) `tools/verify_town_map.js` | **85 / 85**(着手前の実測値と同じ) |
| (C4) `tools/driver_graph_p6.js` | **244 / 244** |
| (C4) `tools/driver_grid_p8.js` | **56 / 0** |
| (C4) `tools/driver_doors_p8.js` | **15 / 15** |

### 実測で分かったこと(依頼書が外していた点を含む)

1. ⚠⚠⚠ **§4 A-2 の `placeUnscaledUi(heroMark, …, -(hm-w), -46)` は dx が誤り。**
   `placeUnscaledUi` は `left = SX(wx + size/2) + (dx − size/2)` を書くので、
   `dx = −半幅` だと **スプライト半分ぶん左へずれる**。正しくは `dx = size/2 − 半幅`
   ([index.html:14606](../index.html#L14606) の敵の装備バッジ `displaySize/2 − 18` と同じ規則)。
2. ⚠⚠ **`dy = −46` では名前ラベルと 1px しか離れない。** ▽ は bob で最大 +4px 下がるので、
   下端が `−46+13+4 = −29`、ラベル上端が `−28`。実測して **`−50` へ**(実測 gap = **5.0px**)。
3. ⭐⭐ **説明文は「紙のまだら模様に食われて読めない」。** 依頼書の
   `--sp-ink-soft: #5a4526` + `opacity: 0.85` は desktop のスクショで判読不能だった。
   → `opacity` を**外し**(斑な紙では透かした瞬間に死ぬ)、色を **`#4a3413`** へ、
   さらに **クリームの薄掛け `--sp-wash: 0.30` を 1 層**足した。⭐ 暗幕 (`--sp-veil`) とは別レイヤ。
4. ⭐ **`max-width: 210px` では説明文が入らない。** 最長の説明(16 文字)は実測 **208 layout px**、
   padding 込みで 240px 要る → `max-width: 280px` / `padding: 6px 16px 7px` にした。
   3 枚の矩形の交差は **0px²** のまま((B6) 緑)。
5. ⭐ **desktop 1440x900 の実効 zoom は 0.8667**(依頼書の「およそ 0.88」はほぼ当たり)。
   ⚠ 説明を依頼書どおり 12px にすると実効 **10.40px** で、受入条件 (B5) の 10px に
   **0.4px しか余裕が無い**。**13px(実効 11.27px)** を採用。名前も 16→**17px**。
6. ⚠⚠ **廃坑は `camZ ≈ 0.346`(卓上グリッド)。ワールド px と画面 px を取り違えないこと。**
   装置でヒーローを「ワールド 240px」横へ引き離したら画面上は **83px** しか離れず、
   閾値 100px の (Z2d) が一度赤くなった(実装の欠陥ではなく**装置の誤り**)。
   → 引き離しを 480px にし、閾値は **(A3b) の下限から導く**(2 つが別々に漂わないように)。
7. ⚠⚠ **負のコントロールのアンカーは「どのファイルの話か曖昧でない行」を選ぶ。**
   `platehide` の初版アンカー `      transform: translate(-50%, -50%);` は
   **index.html にも在り**、(Z3) の装置 assert が実行前に止めた。
   → `        s.style.top  = p.y + "px";`(town.html 固有)へ差し替え。
8. ⭐ **`?signplate=0` は変更前の丸と矩形が完全一致**(`x=637 y=123 55x55` = 着手前の実測値と同じ)。
   丸のスタイルは「消す」のではなく `body.signplate-off` で**厳密に復元**している。
9. ⭐ **依頼書 §2 の実測はすべて再現できた。** 頭の天辺 = 箱の上端 + 32px(街の実測 gap = 8.0px)、
   `tryPromoteNewHead` による ally DOM の `remove()`、`verify_town_map` の 4 つの縛り、
   紙の流用(新規アセット 0)。⭐ ▽ を 1 枚だけ作る設計のおかげで (A4) は素直に緑になった。
10. ⭐ **compact 390x844 では zoom が上限 1.5 ではなく 0.6979**(HUD 帯 111px を引いた
    可視域を満たす倍率)。札は画面幅の約 42% を占めるが、compact の導線は
    `#townHud` ボタン(絵文字付き)のままなので (B7) は緑。⚠ **実機での見た目は未確認**。

### 残った宿題

- ⚠ **iOS 実機での目視**(▽ の大きさ・立て札の可読性・compact での札の占有率)。
  headless では desktop 1440x900 と compact 390x844 のスクショまで。
