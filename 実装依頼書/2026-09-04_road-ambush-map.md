...
⭐ **#51 の実装窓へ「これらを 1 つの名前つき定数 `AMBUSH_FIELD` に束ねてほしい」と依頼済み**
(2026-09-04 `SendMessage`)。⇒ #52 は **そのオブジェクト 1 つを差し替えるだけ**で済むはず。
⚠ 着手時に **実際にそうなっているかを確認する**。なっていなければ、まず束ねる作業から始める。
...
`verify_road_ambush`(#51 の値)/ `driver_mapdef_step1` / `driver_graph_p7` /
`driver_field_step7` / `verify_road_events` / `verify_road_boon` /
`verify_world_steps` / `verify_world_map` / `verify_quest_walk`

⚠ **`driver_mapdef_step1` の golden 8 件のうち `caravan-road`(横持ち/帯マスクなし)だけが
特殊な扱い**(`index.html:8325` が明記)。ここは特に注意して見る。
⛔ `verify_player_sheet` の FAILED `{(2c)(2d)(8a)(8f)}` は #48 の担当(集合が同じなら非退行)。
