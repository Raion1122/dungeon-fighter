...
⛔ **筆頭 = 非退行が未決着**。項目6 の再走で**真の 緑→赤 が 2 本**残っている(§12-6 (b))。
どちらも**本番ではなく測定器**の腐りで、直し方まで実測で分かっているが、
**assert の意味を変える判断が要る**ので本チケットでは手を付けていない。

| 残件 | 直し方の当て(実測済み) |
|---|---|
| `driver_doors_p5 (1x-c2)` | 「共通組を持つ舞台ペアの**過半**」は**母集団の形を焼いた定数**。#62 が `(1x-a)` にやったのと同じく**台帳から導く**形へ言い直す(例: 「共通組を 2 組以上持つペアに限れば全ペアで食い違う」など、畳みで扉が減っても腐らない書き方)。⛔ 「過半 → 1 組以上」へ下げるのは写経で、F4 竜の巣(#67)でまた腐る |
| `verify_swamp_novice (4b)` | `THEME_EXCEPTIONS` へ `'undead-temple': '#66 (2026-09-11) 神殿を卓上大部屋 2 枚へ畳み n4big / n7big を追加'` の **1 行**。⭐ 装置 assert `(4b2)` が「例外表のテーマが実際に差分を持つ」ことを要求するので免罪符のばら撒きにならない(#63 が砦で同じことをしている) |
...
**#66 は本番と受入を 6 項目・7 コミットで着地させた**(`b7501aa` / `356d22b` / `2de59cb` + `09988f0` /
`c07c805` / `e664dc7` / 本行を書いたコミット)。神殿は **8 ノード → 2 ノード**、旧タイプ MAP 全廃は
**F1 沼 / F2 砦 / F3 神殿**が済み、残りは **F4 竜の巣(#67)** のみ。

- 受入 `tools/verify_temple_fold.js`(新規・base **10261**)= 素 **24/24** / `--negative` **7/7 空振り 0**
  (項目6 の再走でも **EXIT=0 / 168 秒**)
- 既存 golden は項目5 で **3 本を言い直して全部緑**(249/249 + 67/67 + 119/119)。
  ⛔ **期待値を下げた箇所 0 件 / golden データの焼き直し 0 件**
- ⛔ **ただし非退行は未決着** — 母集団 138 走行の突き合わせで**真の 緑→赤 が 2 本**(§12-6 (b))。
  ⭐ どちらも測定器の腐りで本番の欠陥ではないが、**#66 は「緑→赤 0 本」を達成していない**。
- ⭐⭐⭐ 本チケットで一般解として残るもの:
