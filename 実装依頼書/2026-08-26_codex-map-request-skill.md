...
**完了**(2026-08-26)。⭐ 起草時の主張のうち **6 件が着手前/実装中の実測で崩れた**ので、
本節に訂正を残す(依頼書の本文 §4/§8 はそのまま = 起草時の記録として保存)。

### 作ったもの

| ファイル | 状態 |
|---|---|
| `tools/make_grid_map.py` | `--fit` / `--fit-around` を追加(+124 / -2 行)。⛔ `GRIDS` / `bake()` / `verify()` / `TOL_*` / `DEFAULT_TILE` は 1 行も動かしていない((3a)(3b)(3c) が機械証明) |
| `tools/verify_codex_map_skill.js` | 新規 574 行。Node のみ・`--negative` 内蔵・変異 5 本 |
| `.claude/skills/codex-map-request/SKILL.md` + `references/` 3 枚 | 新規 |

⚠⚠⚠ **スキル 4 枚は git に入らない。** `.gitignore:2` が `.claude/` を丸ごと除外している
(`git ls-files .claude` = 0 件。既存の `impl-request` / `dev-meeting` も同じく未追跡)。
本チケットの commit に含まれるのは `tools/` の 2 ファイルだけ。
⭐ 3 スキルまとめて版管理するかは**別チケットの判断**(ここで .gitignore を変えると
本チケットの範囲を越える)。
...
| # | 起草時の主張 | 実測 | 採った形 |
|---|---|---|---|
| 1 | §4 のコード片 `center = ... else (float(tile) ...)` と 受入条件 **(1b)** が**両立しない** | `--tile 64` を探索中心にすると窓は 60.16〜67.84px。周期 **45.70** の廃坑入口は原理的に入らない | `--fit-around` を第一の中心にし、`--tile` は**明示的な近道**として残す。(1b) は `--fit-around 46.55 --tile 64` で「tile に張り付かない」を測る |
| 2 | 「中心未指定なら 32〜120 を粗く走査」 | ⛔ **3 方式とも失敗**(下表) | 中心を**必須**にし、無いときは `SystemExit` で**中心の出し方を教える**(メッセージ自体が罠 B の教材になっている) |
| 3 | 探索窓 ±6% | 15 マスの軸では 1 マス数え違えると **6.7%** ずれて窓から出る | **±8%**(`FIT_SPAN_RATIO`)。23 マスを 24 と数えても 4.3% で入る |
| 4 | (3a) は「着手前に控えた SHA-256」と比べる | 焼き直しは **`assets/` の現物 4 枚とバイト一致**した | 基準を**コミット済みの納品物そのもの**にした。控えた値の写経が消え、将来も生きる |
| 5 | 変異 `orphanref` =「リンクを 1 本消す → 孤児が 1 件」 | SKILL.md は各 reference を **2 箇所から**参照しているので 1 本消しても孤児にならない | 「**実在しないファイル名へ壊す**」に変更。(4b) はリンク切れ/孤児の両方を見るのでどちらでも赤くなる |
| 6 | (1b) の許容差 period ±0.01 / phase ±0.05 | 中心が違うと粗探索の着地点が動く。実測 **dT ≤ 0.014 / dPh ≤ 0.15** | (1a) は厳しいまま(**6/6 完全一致**を実測)、(1b) だけ **±0.02 / ±0.20** へ。⛔ ここを 0.01 に締めるのは「素材に存在しない精度」の要求 |

#### ⚠⚠⚠ 罠 D(新規) — 中心無しの広域探索は倍音を拾う

`comb_score` は「櫛が当たった画素の**平均**」なので、**周期が大きいほど標本が減って平均が上がる**。
つまり **異なる周期どうしを比較できない**。3 方式を実測して全部外した:

| 方式 | 廃坑入口 縦 (正解 45.70) | 港町 縦 (正解 63.945) |
|---|---|---|
| `comb_score` の素朴な argmax (32〜120px) | **91.40** (= 2 x 45.70 の倍音) | **96.02** |
| 反櫛との差 (contrast。倍音は原理的に潰れるはず) | 45.69 ✅ | **95.92** |
| 離散フーリエの基本波 (2T に山は立たない) | 46.4 → 45.83 | **32.4** |

⭐ 港町は**基本波より 32px 付近の成分のほうが強い**ので、どの汎用手法でも当たらない。
だから中心は人が与える(「素材の幅 ÷ ざっと数えたマス数」)。
⚠ この事実は `make_grid_map.py` の `--fit` 節と `references/grid-fit-recipe.md` の両方に
数値ごと書いてある(次に「広域探索にすれば親切では」と考える人を止めるため)。

⭐ **`--fit` は 4 素材 8 軸すべてで台帳のマス数を正しく復元した**(中心を ±1 マス変えても同じ)。

### 実測(すべて本窓で実行)

| 装置 | 結果 |
|---|---|
| `node tools/verify_codex_map_skill.js` | **17/17 PASSED / FAILED 0 / PENDING 0** |
| `node tools/verify_codex_map_skill.js --negative` | **21/21 PASSED**・変異 5 本すべて担当節が赤・**空振り 0**・巻き込み 0 |
| `node tools/verify_quest_walk.js` | **25/25 PENDING 0**(非退行) |
| `node tools/verify_quest_walk.js --negative` | **46/46 PENDING 0**(非退行) |
| `node tools/verify_recruit_size.js` | **82/82**(非退行) |
| `--fit harbor... --tile 64` | 台帳 `phlan-harbor` の **6 数値と完全一致**(33.40 / 7.25 / 63.945 / 64.410 / 23 / 15) |
| 既存 4 件の焼き直し | `assets/` の現物と **SHA-256 全件一致**・検算 3 指標 OK |

⭐ 変異 `fitwrite` は本番の `assets/town_phlan.jpg` を実際に焼き直すが、**中身が同じバイト**
なので `git status` は clean のまま(気づけるのは mtime だけ = (1c) が 3 点で見ている理由)。

### 副次的に直したもの

- 冒頭 docstring の「使い方」に `--fit` を追加
- 「★台帳 (GRIDS) の値はどこから来たか」と `GRIDS` 直前のコメントが、**既に消えている
  `fit_grid.py` を「scratchpad へ戻して回せ」と指示していた**ので、`--fit` を指すよう訂正
  (残すと次の読者が同じ罠を踏む)

### changelog

**呼んでいない。** `scripts/hooks/check_changelog.py:24` の `GAME_LOGIC` は
`("index.html", "tavern.html", "audio.js")` で、本チケットが触るのは `tools/**` と
`.claude/skills/**` のみ。フックは鳴らない(§2-7 の予告どおり)。

### 次(#25)への申し送り

- #25 の STEP1 は **`--fit-around` を必ず渡す**こと。中心 = 素材の幅 ÷ ざっと数えたマス数
- ⭐ 発注文に**マス数を書かない**。書くのは「1 マス 64px 相当の格子で 1536x1024」まで
- #25 の「実装結果」に **「スキルのどこが足りなかったか」を必ず書く**(実地試験を兼ねる)
