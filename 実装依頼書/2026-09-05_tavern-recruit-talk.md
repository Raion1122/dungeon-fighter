...
### 12-2. STEP3 着地 — 声を掛けて同行候補にする + 単身出発の確認

#### ⚠⚠ 12-2b. 「単身出発の確認」の経緯(⛔ 依頼書と食い違って見えるので必読)

**これはユーザーが実装中に、実装窓へ直接指示したもの**(2026-09-05)。起草窓は関与していない。

⚠⚠⚠ **記号 "C" が 2 つあるので注意。** 実装窓は当初コミットに「ユーザー決定 C」とだけ書いたが、
これは**誤読を招く書き方だった**(起草窓の指摘で訂正)。2 つの A/B/C は**別の設問**である:

| 出所 | 設問 | ユーザーの答え |
|---|---|---|
| **起草窓** | Lv1 の呪文枠 3 個をどう配るか(C = `SPELL_SLOT_CURVE_MAGE[1]` を 3→4) | **(A)** `magic-missile ×2 + sleep ×1` |
| **実装窓** | 「候補 0 人ならソロ」で golden 7 本が赤くなった件の身の振り方 | **(C)** |

実装窓が示した 3 択と、選ばれた (C) の中身:

- (A) 決定どおり進め、golden 7 本(約 62 assert)を新モデルへ書き換える。⛔ 難易度の崖は残る
- (B) 誘った人 + 残り枠は従来どおり自動で埋める。golden はほぼ緑のままだが「誰も誘わなければソロ」に反する
- **(C) = (A) + 出発前に一度だけ確認を出す** ← 採用

⭐ 根拠として提示した実測(§9): **4 人編成ですら沼で 6/6 敗北**し、しかも仲間 3 人が健在なまま
**主人公だけ先に落ちている**。ソロには肩代わりの余地がゼロなので、酒場で誰にも話しかけなかった
プレイヤーは**警告なしに単身で潜って死ぬ**。

#### ⚠ §11 の「⛔ ゲートは作らない」との関係

食い違わない。§11 が禁じたのは **進行を止める通せんぼ**(「誰も連れずには出発できない」)。
ここで足したのは**一度きりの案内**で、「一人で行く」を押せば必ず出発できる。
⭐ 一度きりの器は `prologueSeen` / `prepOnboardingSeen` と同じ枠(`soloWarnSeen`)。
⛔ 後から読む人が「起草が禁じたものを実装が入れた」と誤読しないよう、ここに残す。
...
### 12-4. STEP4 着地 — 受入ドライバ `tools/verify_recruit_talk.js`

**素 25/25 PASSED / FAILED 0 / PENDING 0 ・ 負のコントロール 11/11 本が期待どおり。**

#### ⚠ ポートは依頼書指定の 9940 から **10020** へ移した

⭐ 9940 は **#50 `verify_cone_cast` が 9940〜9960 を占有済み**で衝突する。
さらに #51 `verify_road_ambush` が **9970〜9990** を予約している(`tools/*.js` の base 最大は 9970)。
⇒ base **10020** / 変異 10021〜10031。

#### 測っているもの(25 件)

| 節 | assert |
|---|---|
| §0 | (0a) 卓に 4 人 ⭐ 無いと全 assert が空振りして永久緑 / (0b) 候補を実体から引く |
| §1 | (1a) 主人公経路 / (1b) NPC 経路 / (1c) mm を失わない / (1c2) **(A) の配分** / (1d) `SKILL_SLOT_CURVE` 凍結 / (1d2) **`SPELL_SLOT_CURVE_MAGE` 凍結** / (1e) 他職不変 / (1f) 既存セーブ / (1g) 酒場側の写し |
| §2 | (2a) 声掛け→候補 / (2b) 誘った人が編成に / (2c) 誘っていない人は出ない / (2d) ソロ / (2e) **マッチング演出に名前** / (2f) 解散 / (2g) 上限 / (2h) **単身確認** |
| §3 | (3a) `npc-crowd` の座標恒等 / (3b) 人数表示と実体の一致 |
| §4 | (4a) `?recruittalk=0` / (4b) `?magesleep=0` / (4c) **独立性** / (Z) JS エラー 0 |

⭐ **期待値を焼いていないもの**: 上限 = `RECRUIT_MAX` / 依頼の重さ = `recruitCountOf` /
人数 = `selection.partyMembers` / `npc-crowd` の座標 = **配信バイトから導出**。

#### ⭐⭐⭐ 負のコントロールが受入条件の穴を 1 つ暴いた(#53 の教訓が効いた実例)

初版で **`sleepall` が空振り**した(期待 `[1e]` / 実際に赤 `[]`)。握り潰していたら
**受入条件が永久緑のまま出荷**されていた。真因は 2 つあり、**両方**直した。

1. **変異側**: 非魔法職に sleep を index 1 で挿しても、Lv1 の枠 1 個の `slice(0,1)` に
   切られて **観測点に現れない**。⇒ index 0 (`unshift`) にした。
   ⭐ 教訓 = **変異は「注入できたか」ではなく「測っている場所に現れるか」まで設計する。**
2. **assert 側**: (1e) は「素の腕 vs `?magesleep=0` の腕」の差分で見ていたので、
   **`?magesleep` でゲートされていない漏れは両腕に同じように出て検出できない**。
   ⇒ 「非魔法職の `equippedSkills` に sleep が 1 件も無い」を**直接**併せて見る形へ強化。

#### ⚠ 実装中に踏んだドライバ側の罠 3 件(製品は無傷)

1. **`#partyMatch` という id は存在しない** — 正しくは `#partyMatchOverlay`(中身は `#pmColumns`)。
2. **`tile` / `dx` / `dy` を 1 本の正規表現でまとめて拾うと、`tile` を持たない `stroll` 型で
   省略可能グループが崩れて全件 `tile:null` になる** ⇒ エントリごとに切って個別に読む。
3. ⭐⭐ **受注ナレ (`#prologueOverlay`) が音声ペースで約 20 秒**かかり、クリックで送らないと
   マッチング演出に到達しない。⛔ 固定 `sleep` で待つ実装は必ず空振りする
...
コードと自動検証はすべて着地。以下は**人が遊んで決める**もの:

1. ⭐⭐⭐ **ソロで潜って詰まないか**(勝てるかではなく、敗北して帰還できるか)
2. 卓の冒険者のタップ領域が 44px 以上あるか(縦持ち)
3. 承諾のトーストが既存の雑談キューを詰まらせないか
4. 縦持ちで「同行の約束: N / 3 人」が読めるか
5. Lv1 の魔法使いがスリープを撃つのを実際に見る
6. 単身出発の確認の文面と出るタイミング
