...
- **起草**: 2026-09-07(計画窓) / **ステータス**: **完了**(2026-09-07 実装窓が着地・起草窓がコミット)
  (承認 = 2026-09-07 ユーザー「判断は任せます」)
...
- ✅ **`実装依頼書/README.md` への行追加は着地後に実施済**(2026-09-07)。起草時に用意していた行:
...
## 12. 実装結果(実装窓・2026-09-07)

### 12-1. 着地したもの

| ファイル | 変更 |
|---|---|
| `tavern.html` | 基底 `#pmDrawer` から `max-height`/`overflow-y` を削除 + `body.pmFitOff` 復元規則 / compact の 30vh を撤退時だけへ / `PM_FIT_ON` / `pmScrollBeforeOpen` + 開いたら寄せる・閉じたら戻す / changelog 1 行 |
| `tools/verify_pm_drawer_fit.js` | **新規**(port 10081)。79 assert + `--negative` 9 本内蔵 |
| `tools/verify_party_match_setup.js` | 変異 M6 の張り替え + **`clickCenterOfSel` の修理**(§12-3 の (5)) |

### 12-2. 受入結果

- 素 **75/79 PASSED / FAILED 0 / PENDING 4**、`--negative` **9/9 空振り 0**
- PENDING 4 は全部「原理的に測れない」を本文に書いたもの:
  **(3b-v1)(3c-v1)** = デスクトップ 1280×900 は引き出しを閉じた `#pmInner` が maxScroll 0 でそもそもスクロールしない /
  **(2b-v4)(2c-v4)** = 390×667。
- 母集団 9 本すべて exit 0(最終バイトに対して実走):
  `driver_action_priority` 92/92 / `driver_depart_menu_clean` **41/41** / `driver_equip_compact_ios` **31/31** /
  `driver_party_view_reopen` 35/35 / `verify_darkvision` 25/25 / `verify_party_match_setup` **36/36** /
  `verify_prep_retire` 30/30 / `verify_quest_walk` 25/25 / `verify_recruit_size` **82/82**。
  ⭐ 太字の 3 本が §2-8 で採り損ねていた件数。
- **着手前の赤の集合 = {M6 空振り} の 1 件のみ → 着地後は 0 件。増えた赤はゼロ。**

### 12-2b. 直りの実測

| 画面 | 引き出しの実高(着手前 → 着地後) | 内側で切れている量 |
|---|---|---|
| デスクトップ 1280×900 | 170px → **638px** | 470 → **0** |
| ノート 1366×768 | 38px → **638px** | → **0** |
| iPhone 縦 390×844 | 22px → **963px** | 943 → **0** |
| 極小 390×667 | → **963px** | → **0** |

折り畳み段を全部開いた最悪ケースでも切れ 0(1002px / 1503px)。
「傾向」段は 4 画面とも器をスクロールすれば完全に画面内。
撤退 `?pmfit=0` は 42vh=378px / 30vh=253.2px にちゃんと戻る。

### 12-3. ⭐ 依頼書が外していた点(実測で崩れた 6 件 + 1)

**(1) ⭐⭐⭐ §5-1 の「`pmRenderDrawer(idx);` の後に `pmScrollBeforeOpen` を採る」は【遅すぎる】**

そこで採ると (3b)(3c) が **v3/v4 で赤くなる**(「開く前=100 / 閉じた後=32」)。
真因はクランプ: 縦持ちでは `inner.classList.add("pmDrawerOpen")` が付いた瞬間に
`#pmHeader` が `display:none` になり `#pmColumns` が **669→423px** に縮むため、
ブラウザが `#pmInner.scrollTop` を 100 → 32 へ丸める。**採れた値そのものが既に汚れていた。**
⇒ 採取を **「DOM を 1 バイトも触る前」へ移した**。`wasClosed` の判定も同じ場所。
⭐ v2(1366px)だけ緑だったのは compact 規則が効かないから。**縦持ちでしか出ない欠陥**。

**(2) §8 (2b)「最下段まで送った同じ状態で装備段の summary も画面内」は 390×667 で原理的に不可能**

装備段は引き出しの**途中**にあるので、最下段(傾向)を画面へ入れると上へ 14.7px はみ出す
(実測 top=−14.7 / bottom=15.3)。⇒ V4 は (2c-v4) と同じく **対象外(理由つき PENDING)** にし、
代わりに **(2b2) 到達性**(器をスクロールすれば summary を完全に画面内へ持ってこられる)を
**4 画面すべてに新設**した。緩めたのではなく、**測る場所を正した上で母集団を増やしている**。

**(3) §8 (5d)「引き出しを一度も開かずに出発したとき 1 バイトも変わらない」は原理的に成立しない**

`departToScenario` が 4 本のキーを**正当に**書く。⇒ 測りたい中身(器の見た目の変更が保存へ
漏れていないか)に合わせ「**開いて閉じるだけでは selection も localStorage も動かない**」へ言い換えた。

**(4) §8 の変異 `rafless` は完全な空振り → 表から外した**

注入しても 75/79 全緑。理由は仕様どおりで、`getBoundingClientRect()` が同期レイアウトを
強制するので hidden を外した直後でも rect は正しい。**Chrome では rAF の有無で 1px も変わらない**。
⇒ 本番から rAF は外していない(実機が iOS Safari で、ヘッドレス Chrome では要不要を判定できないため)が、
コード側のコメントを「⚠⚠ requestAnimationFrame が要る」から**実測どおりの文面へ書き換えた**。
⛔ 「効いている」と読ませないこと。空振りする変異を表に残すと `--negative` が永久 exit 1 になる。

**(5) ⭐⭐⭐ §2-8 の 9 本のうち `verify_party_match_setup` が【素で 6 本赤くなる】**

着手前 36/36 → 着地直後 **30/36**((3a)(3b)(3c)(3d)(4a)(4b))。
真因は #56 の自動スクロールと、あのドライバが **`page.mouse.click` の実座標**で
カードを叩いていることの組み合わせ。1 枚目を開いた瞬間に器が引き出しの頭までスクロールし、
2 枚目以降のカードが器の外(rect.top が負)へ流れるので、クリックが画面外に落ちて
**`pmZone` に当たる**((3a) が「pmOpen=1 なのに self=false」)。
⇒ **assert 本体は 1 行も触らず**、`clickCenterOfSel` に「**画面外にいるときだけ**器をスクロールして
対象を画面内へ戻す」を足した(見えているときは 1px も動かさない = #56 以前と同一挙動)。
これで **36/36 へ回復**。
⭐⭐⭐ 教訓 = **器をスクロールさせる変更は、実座標で叩いている既存 golden を全部壊す。**

**(6) §8 の変異 `maxonly` / `ovback` / `maxback` は【同じアンカーを共有する】**

3 本とも `#pmDrawer` 基底規則の末尾を書き換えるので、最初の `mutate()`(変異後のバッファで
アンカーを数える実装)では 1 本目を注入した瞬間に 2 本目が「注入点 0 箇所」= **偽のアンカー腐敗 exit 3** になり、
**いちばん大事な `maxonly` が 1 度も走らなかった**。
⇒ アンカー健在チェックを **手つかずの原本(`PRISTINE`)に対して行う**よう直した。
その後の実測で `maxonly` は **28 ラベルを赤にする**(§2-1 の罠を機械で守れている)。

**(+) §8 (1b) の「差 <= 2」は border 1px×2 とちょうど同値で、偶然通っていただけ**

`rect.height` は border 込みなので、明示的に border を引いてから比べる形にした(実測の差は 0.2〜0.4px)。

### 12-4. 変異表の実測(⛔ 予想ではなく実走で確定した値)

| 変異 | 担当(赤くなるべき) | 実際に赤くなった数 |
|---|---|---|
| `maxonly` ⭐⭐⭐ | (1a-v1) | 28 |
| `ovback` | (1a-v1) | 28 |
| `maxback` | (1a-v1) | 14 |
| `compactback` | (1a-v3) | 7(V1/V2 は緑のまま = #35 と同じ非対称を再現) |
| `noscroll` | (3a-v1) | 4(3a を 4 画面) |
| `norestore` | **(3b-v3)** | 6 |
| `stickydead` | (4a-v1) | 10 |
| `switchdead` | (6a-v1) | 4 |
| `switchtwice` | (6a-v1) | 1 |
| ~~`rafless`~~ | — | **0 = 空振り。表から削除**(§12-3 の (4)) |

⚠ `norestore` の担当を **(3b-v1) にしてはいけない** —— v1 は閉じた器の maxScroll が 0 で
(3b-v1) が PENDING になるため、担当にすると永久に空振り扱いになる。**縦持ちが唯一測れる画面**。

### 12-5. STEP3 の結果

M6 の張り替えは成功。`--negative --only M6` は exit 0 で、**赤くなったのは (4c) 1 本だけ**。
(張り替え前の下見では巻き添えが 6 件出ていたが、それは (5) の計測装置の欠陥によるもので、
`clickCenterOfSel` を直したあとは (4c) 単独になった。)

### 12-6. 起草窓による独立検証(2026-09-07)

実装窓の報告を鵜呑みにせず、起草窓が作業ツリーの最終バイトに対して測り直した:

| 走らせたもの | 結果 | 所要 |
|---|---|---|
| `verify_pm_drawer_fit` | **75/79 PASSED / FAILED 0 / PENDING 4**(exit 0) | 70s |
| `verify_pm_drawer_fit --negative --only maxonly` | **FAILED 28**(担当 (1a-v1) を含む 28 ラベルが赤)(exit 0) | 71s |
| `verify_party_match_setup` | **36/36 PASSED / FAILED 0 / PENDING 0**(exit 0) | 102s |
| `verify_party_match_setup --negative --only M6` | **35/36 · FAILED 1** = 赤くなったのは **(4c) だけ**(exit 0) | 102s |

⭐ **実装窓の報告と 1 つも食い違わなかった。**
⭐⭐ とくに `maxonly`(= 「max-height だけ消す」素朴な直し方)が **28 ラベルを赤にする**ことを
起草窓の手でも確認した。§2-1 の罠は機械で守られている。
⭐⭐ M6 の張り替えも確認 —— 注入すると **(4c) 単独**が赤くなり、巻き添えゼロ。
着手前は同じコマンドが「空振り: M6→(4c)」で exit 1 だったので、**負のコントロールが生き返っている**。

⚠ フル `--negative`(9 本)は実装窓の実走(9/9 空振り 0)を採用し、起草窓では
`maxonly` 1 本だけを抜き取って確認した(所要時間の都合)。

### 12-7. 残(実機体感)

§9 の 7 項目はそのまま残。とくに ③「開いた瞬間にカード列が上へ流れるのが唐突すぎないか」
(なめらかスクロールにするかは目で決める)と ⑤「デスクトップでカードが見えなくなるのが気になるか」
(気になれば §11 の #57「上を畳む」へ)。
