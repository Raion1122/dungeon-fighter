...
- **ステータス**: **完了 `4090a99`**(2026-08-21 実装。`tools/driver_bgm_mine.js` 37/37 PASS)
- **起草日**: 2026-08-20
...
## 実装結果 (2026-08-21 / `4090a99`)

依頼書のとおり STEP1〜4 を実装。受入条件 §1〜§3 と負のコントロール 3 本は
`tools/driver_bgm_mine.js`(新規)で **37/37 PASS**。
非退行として `driver_graph_sce1` 106/106・`driver_mine_wall` 53/53 も緑を実測。

### 依頼書からの逸脱 (4 点)

1. **`audio.js` に読み出し口 `GameAudio.__bgmFiles()` を足した**(変更範囲に無かった)。
   §3a「`src` のファイルが実在する」を測るのに `BGM_FILES` の実体が要るため。
   ⭐ ドライバ側に表を写経すると、**実装とドライバが同じ間違いを共有して永久に緑**になる。
   既存の `__bgmFileState()` / `sfxNames()` と同じ読み取り専用の口として並べた。
2. **`index.html` の dev シーム `__graphRun` に `bgm()` を足した**(同上)。
   `currentBgmId()` / `sceneBgmId()` **そのもの**を呼ぶ純粋な読み取り。これで観測が
   ①「実際に鳴った側」(`playBgm` を包んで記録) ②「決める側」(このシーム) の
   **2 経路の突き合わせ**になる。片方だけだと写経になる。
3. **§1c の測り方**。玉座まで実際に歩かせるのは遅いので、production の退避口
   `?bossapproach=0`(= 入室で即ボス部屋)で「実際に鳴らした ID」を測った。
   ⚠ それだけだと「入室で即ボス曲」という別の欠陥を温存したまま緑になるので、
   **(1c2) を足した** — 素のページでは n1 に入っただけでは `__inBossRoom` が偽のままで、
   鳴っているのは `mine_depths`。切替点がラッチ側にあることを別の assert で押さえている。
4. **`?minefold=0` の n7 は大部屋ではなかった**(`bigRoom=false`)ので、旧構成では
   入室と同時に `mine_boss` が鳴る。畳み込み後の n1(大部屋)だけが接近ラッチを通る。

### ⚠ 測定器の落とし穴 (実測)

**ノード遷移の直後に `playBgm` のログを読むと、1 ノード前の記録が返る。**
`enterNode` は `currentNodeId` を先に書き換えるが、曲を鳴らす `setPhase("explore")` は
`placeNodeParty` → `snapCamera` → 明転 300ms の**後**。`nodeId()` の一致だけを合図に
読むと「まだ n0 の `mine_entrance`」を掴む(初回実行で §1b/§1c/§1d の 4 本が赤くなった。
**実装ではなく測定点の誤り**)。→ `waitPlayedAt()` で「そのノードでの記録が出るまで」
ポーリングする。固定時間の `sleep` にしないのは、健全な分布が窓をまたいだ瞬間に
間欠フレークになるため。

### 未決事項(ユーザー待ち)

- **3 曲の出典**。`BGM_FILES` の `credit` は 3 曲とも **空のまま**。
  ⚠ さらに `audio.js` の設定モーダルに**画面へ出るクレジット行**があり、現在は
  `BGM  魔王魂 / ユーフルカ` の 2 者だけ。出所が別なら**ここも直す必要がある**
  (依頼書起草時は「credit を出す UI は無い」と書かれていたが、実際にはあった)。
- **試聴**。⚠ `file://` 直開きでは音が出ない。`ゲームを起動.vbs` か http で起動して、
  ①切替が唐突すぎないか ②3 曲の音量差 ③d1 は 66 秒と短いのでループの継ぎ目、を確認する。
