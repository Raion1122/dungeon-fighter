...
- **ステータス**: **完了 `7c59c44`** (2026-08-20)
  - 新規 `tools/driver_mine_wall.js` **58/58**。実プレイ 1 周で戦車が乱入し (体の壁 0 マス)、
    5 回突進して 214s で `dungeonCleared`。目視補助 `tools/probe_paint_overlay.js` を追加。
  - ⊙ 依頼書からの逸脱 3 点 (いずれも実測で先の案を却下したもの):
    1. 湧き位置の物差しを「ボスから最も遠い」→「**道のりが 3〜4 ターン**」へ。
       畳んだ n1 では最遠点が入場地点 (19,11) で、ボスまで 40 歩 = 10 ターンかかり
       企画の「猶予 3〜4 ターン」が壊れた。さらに直線距離では道のりと 16 vs 28 歩とずれる。
    2. 「体が収まる」だけでは不十分。最初の実装は 3x2 がちょうど嵌まる窪み (36,15) を選び、
       **8 方向どこへも動けない戦車** (30 ターン全部 BLOCKED) を生んだ。
       → 体が収まらないタイルを `avoidTiles` へ渡して **本番の aStar を「体で走れる道」の探索に使う**。
    3. §1a は「外周が 1 マスも歩けない」とは**言えない**。出口タイルは門番が必ず通すので
       n0=3 / n1=2 マスが残る (塞ぐと部屋から出られなくなる)。→ 「歩ける外周は出口タイルだけ」
       ＋「**歩ける外周どうしが隣り合わない**」(= 帯として通り抜けられない) という不変条件へ言い直した。
  - ⚠ 測定器側の張り替え 4 本 (期待値は書き換えず、旧経路へ固定か実経路へ移動 + 装置 assert):
    `driver_paint_blocked` (変異アンカー) / `driver_grid_p8` (9g) / `driver_grid_p4` §7 /
    `driver_graph_run` (1 引数呼び + (9j) の col 68)。
  - ⚠ 別件で回収した既存の赤: `driver_grid_p7` (3a) の小ノード母集団 → `15d76ef`。
- **起草日**: 2026-08-20
