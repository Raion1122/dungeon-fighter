...
### 12-4. 総括(項目5 / 2026-09-19・実装窓 `claude-a1` セッション `f2b13284-…`)

⛔ **本番(`index.html` / `tavern.html` / `audio.js`)も `tools/*.js` も 1 バイトも触っていない**(走らせて突き合わせただけ)。
前後とも `git hash-object index.html tavern.html audio.js` が HEAD の blob(`f9258a74…` / `e21b7309…` / `b3aaa1fd…`)と一致。
⇒ changelog は不要。触ったのは本 `.md` と `実装依頼書/README.md` の **#70 行だけ**。
使い捨ての走査器・抽出器・比較器は scratchpad `…\scratchpad\item5\`
(`sweep70e.py` / `fp70e.py` / `cmp70e.py` / `armlist.json` / `result70e.tsv` / `logs/` / `rerun/` / `verdict70.txt`)。

#### (a) 母集団と走行数 — ⛔ 定数で焼かず毎回導出した

`base70/pop70.py` を **本コミット時点の `tools/` で回し直した**(項目1 の結果を写経していない):

| 段 | 引き方 | 項目1(140 本のとき) | **項目5(実測)** |
|---|---|---|---|
| — | `tools/*.js` 総数 | 147 | **148** |
| 段1 | 触る語 8 つ | 13 | **14**(`verify_spell_off` が増えた) |
| 段2 | `index.html` / `tavern.html` をコードで読む本 | 138 | **139**(同上) |
| 段3 | 他の `tools/*.js` を**ソースとして読む**測定器 | 5 | **5**(⭐ 増えていない) |
| — | **union** | 140 | **141** |

- union の差分は `verify_spell_off.js` の **1 行だけ**(`diff` で確認)。
- ⭐ **段3 が増えなかったことを、語ではなく振る舞いで裏を取った**:
  (1) `verify_spell_off` を**参照する**他の本 = **0 本**、(2) `tools/` を**ディレクトリとして列挙する**本 = **0 本**
  (`readdirSync` を持つ 4 本 —— `_pptr_profile`(Chrome プロファイルと TMP)/ `probe_party_size`(ROOT と `js/`)/
  `verify_mercenary_roster`(`js/`)/ `verify_codex_map_skill`(`assets/` と `requests/`)—— はどれも `tools/` を見ていない)。
  ⇒ **新規ドライバは既存のどの本にも届かない** = 項目3 の腕を流用してよい二つ目の根拠。
- **走行数 = 151**(`baseline70.tsv` の `in_pop70=yes` が **149 行** + 新規 `verify_spell_off` の 素 / `--negative` の **2 腕**)。
  「union のうち基準に 1 行も無い本」を機械で引くと **`verify_spell_off` ただ 1 本** ⇒ 新規腕の本数も導出値。
- **実際に手元にある証拠は 156 腕**(151 + 項目3 が基準行なしで余分に回した `--negative` **5 腕** =
  `pm_drawer_fit` / `party_match_setup` / `recruit_talk` / `darkvision` / `prep_retire`)。

#### (b) 流用の可否 = **可**(20 腕)。根拠は blob OID

`e6692b0`(項目3)→ `7ea896c`(項目4)の差分は `git diff --name-status` で **`A tools/verify_spell_off.js` と `.md` 2 本だけ**。

    $ git rev-parse e6692b0:index.html  7ea896c:index.html   → f9258a74…(同一)
    $ git rev-parse e6692b0:tavern.html 7ea896c:tavern.html  → e21b7309…(同一)
    $ git rev-parse e6692b0:audio.js    7ea896c:audio.js     → b3aaa1fd…(同一)
    $ git rev-parse e6692b0:js          7ea896c:js           → b77037d0…(同一)
    $ git ls-tree -r e6692b0 -- tools  と  7ea896c の同じ出力を diff
        → 342 本 vs 343 本。差分は **verify_spell_off.js の 1 行の追加だけ**(既存 342 本は blob OID まで同一)

⇒ 項目3 の 25 腕(144.7 分)をそのまま証拠に採り、**残り 131 腕を全数直列で実走**した。
⚠ 本走査の途中で起草窓が `06ec933` / `e5fe167` を積んだが、どちらも `実装依頼書/*.md` だけで
**`tools/` ツリーの OID は `348e6a4f…` のまま不変** = ドライバが見る世界は最後まで byte 同一。

#### (c) 走らせ方と所要

- ⛔ **並列にしていない**(全 131 腕を 1 本ずつ直列)。⛔ **`timeout` で包んでいない**(#64)。⛔ 8765 は落としていない。
- 走査器 `item5/sweep70e.py` は腕を **毎回 `baseline70.tsv` + `pop70_union.txt` から導出**し、
  項目3 の `result70c.tsv` に在る腕を引く(⛔ 腕リストを定数で焼いていない)。軽い順に並べ、重い本を最後に置いた。
- ⚠ `probe_party_size` は自力で終わらないので **600 秒で明示的に taskkill**(#69 と同条件 → exit 1。基準と一致)。
- **項目5 の実走 = 131 腕 / 12,741.0 秒 = 212.3 分**。色が動いた本の再走 = 4 走行 482 秒 = 8.0 分。
  ⇒ **項目5 だけで 220.3 分**、項目3 の 144.7 分を足すと **365.0 分**。
  ⚠ 見込みは「270〜300 分で 141 本」だったが、実際は**流用が効いたぶん 131 腕 212 分**で収まった
  (重い 6 本 —— `bolt_aim` 素 2230.7s / `prep_retire --negative` 1871.7s / `hold_pair` 素 904.0s ほか —— が全部流用側にあったため)。

#### (d) 2 経路の突き合わせ — **#70 由来の緑→赤 0 本**

⭐⭐⭐ **比較器を使う前に「変えていないはずの本で差 0 か」を通した**(項目3 の教訓)。
ただし項目3 の抽出器 `fp70.py` は **素の 13 本でしか自己検証していなかった**ので、母集団 141 本では通用しなかった。
実測で落ちていた書式は **10 個**あり、#69 の凍結指紋から逆算して `item5/fp70e.py` に入れ直した:

| # | 落ちていたもの | 実例 |
|---|---|---|
| ① | 判定マーカー `✅` / `❌` | `driver_monsters_orc` は **判定行 0 行**に見えていた(実際は 7 行) |
| ② | 小文字 `ok` | — |
| ③ | **数字始まりの裸 id**(`0.1` / `1.2`) | `driver_cast_circle` は **id 0 個 / 基準 56 個**。⚠⚠⚠ #67 の「48.2% を見落とす」と同型 |
| ④ | `§` 見出し + 2 トークン目(**ASCII 英数 `_ . -` のときだけ**) | `§0 0a` / `§0 0-1` / `§E Ea` / `§1 DEFAULT_DUNGEON` は足す。`§1 1920×1080`(`×`)・`§E 意図しない`(和文)は足さない |
| ⑤ | **裸の数字だけは id にしない** | `PASS 30 / FAIL 0` という**総括行**が判定行に混ざる(`verify_swamp_fold` が +1 行) |
| ⑥ | `[…] ✓ <tag> OK` 形式 | `[driver] ✓ nocap OK` / `[  1/3] ✓ run1 →`(⚠ 括弧の中身は英字とは限らない) |
| ⑦ | `--- FAILED ---` / `FAILED:` / `[drv] FAILED:` の**再掲ブロック** | #69 はここを `status=RECAP` で数えている(`verify_walk_block` / `driver_speech_v2` / `driver_sce1_events`) |
| ⑧ | `[driver] FAILED: (A1) …` の **1 行再掲** | `driver_speech_v2` |
| ⑨ | マーカー直後に空白が無い形 | `driver_mapdef_step2` の `ok={"ok":true,…}` |
| ⑩ | 選択肢を**長い順**にしないと `PASSED` が `PASS` + `ED` に割れる | 一時的に **14 本で id `ED` が大量発生**した(⚠ 直した) |

- ⭐ **再掲ブロックは判定行の多重集合にだけ入れ、id 指紋には入れない**(入れると id 側だけ二重になる = 実測)。
- 自己検証: 項目3 の **14 腕で新旧どちらの抽出器も差 0**(`cmp70e.py --selfcheck`)。

**結果(`item5/verdict70.txt`)**

| 見たもの | 実測 |
|---|---|
| 突き合わせた腕 | **156**(指紋のある 131 + 指紋の無い 25 は exit + 総括行) |
| exit が基準と一致 | **146 / 149**(基準行のある腕)。食い違い 3 本はすべて下で決着 |
| **経路1 新 FAIL id** | **2**(`verify_run_chronicle` の `(1c)` `(1z1)` のみ) |
| 経路1 消えた id | **0** |
| 経路1 増えた id | **10**(内訳は下表) |
| **経路2 新 FAIL 行** | **2**(同じ `verify_run_chronicle` の 2 行) |

**割り付け(⛔ 1 件も未分類を残していない)**

| 本 | 差 | 決着 |
|---|---|---|
| `verify_run_chronicle` | 基準 exit 0 → **1**(73/73 → 71/2)。新 FAIL id 2 / 新 FAIL 行 2 | ⭐ **フレークだった。再走 2 回とも `73 PASSED / 0 FAILED`・exit 0 で、凍結指紋と id差 0 / 行差 0**。赤い側の 2 本は `(1z1)` = **装置** assert で、`enemies` にまだ `alive` が 1 体以上残っている瞬間に触る必要がある(オートプレイが先に掃討し終えると成立しない)。⛔ この本は `partySkills` / `spelloff` / `clericSpellsOff` を **1 箇所も参照していない**(grep 0 件)= #70 の経路を通らない |
| `driver_speech_engine` | 基準 exit 1 → **0**(赤→緑)。id差 2 / 行差 3 | ⭐ **フレーク**。再走 2 回とも **exit 1 / 16 PASSED・`(4) カメラが実際に動いた` が赤**で、凍結指紋と **差 0**。⚠ §12-0 (c2) が「**安定した**非緑」に分類していたが、実測では両方向に転ぶ(`camXレンジ` 14.5px / 10.1px) |
| `driver_field_wagon` | 基準 exit 1 → **0**(赤→緑)。id差 3 / 行差 3 | **既知フレーク**(§12-0 (c2) の 5 本のひとつ)。差は `P4-iphone_land` の PASS / FAIL 反転のみ |
| `driver_grid_p5` | 行差 **16**(exit 0 / 判定 103 本とも全部 PASS) | **仲間の抽選**。基準は `ally:cleric` / `ally:rogue`、今回は `ally:dwarf` / `ally:elf` —— 同じ assert が職名を本文に持つため。⭐ 判定の本数も色も不変 |
| `probe_s2_clear` | 行差 **2**(exit 0) | **オートプレイの乱数**。`残敵=8`(`#`)と `残敵=10`(`##`)の桁が変わっただけ(3 走行のうち 1 本) |
| `driver_mapeditor_props` / `driver_mapeditor_painting` / `driver_skillcheck_roster` / `driver_field_step2` / `driver_mapdef_step2` / `probe_s2_clear` | **id 差のみ 計 7**(⚠ **行差はすべて 0**) | **比較器の残差**。`§E 意図しない` / `母集団:` / `D2*` / `={"ok":true,…}` など、#69 の id 抽出規則を完全には復元できなかった行。⭐ **同じ本の経路2 が差 0** = 判定行は正規化後に完全一致 ⇒ 内容は同一と言い切れる |

⇒ **#70 由来の緑→赤 = 0 本**。**意図した差分は新規 `verify_spell_off` の 2 腕だけ**
(素 **51/51 PASSED  FAILED 0  PENDING 0** exit 0 / 24.4 秒、`--negative` **36/51・12 本すべて担当ラベルが赤(空振り 0)** exit 0 / 292.9 秒)。
⚠ 着手前から赤の 2 本(`driver_action_priority --negative` exit 1 / `probe_party_size` exit 1)は
**逐語まで基準と同一**で、⛔ 直していない(§11)。引数ガードの即死 exit 3 = 3 本も基準どおり。
⭐ **影のツリーは組まずに済んだ**(#69 と違い、色が動いた 3 本がすべて再走だけで決着したため)。

#### (e) ⭐ 依頼書の主張が崩れた点 — **31 件**(#68 は 25 件 / #69 は 17 件)

§12-0 〜 §12-3 に散っていたものを 1 箇所へ集約する。

| # | 出所 | 崩れた主張 | 実測 |
|---|---|---|---|
| 1 | §2-3③ / §5-5 | `hasPreset` は **1 行** | **2 行**(変異 `presetlen` は 2 行の逐語で引く) |
| 2 | §2-4 / §5-6 | `.slice(0, skillSlotsForLevel(10))` は 1 箇所 | **2 箇所**(もう 1 つは ⛔ 触らない戦士の旧キー移行) |
| 3 | §2-7 | 母集団の第1段は名指し **11 本** | **13 本**(`magesleep` で chimera / griffon が入る)⇒ 項目5 では **14 本** |
| 4 | §4-4 / §8 (2b)(5a) | 12 個配分 → 開き直して **5 個** | **素では 6 個**(切り詰めの後ろで #54 の差し込みが 1 個足す) |
| 5 | §8 (0c)(3a) | 僧侶の期待値 = `getClericSlots(5)` の **8 キー** | **4 キー**(`isSpellKnown` の習得ゲートが後段で効く) |
| 6 | §2-9 | `index.html` 39,539 行 / `tavern.html` 10,586 行 | **39,595 / 10,599**(#69 で +56 / +13) |
| 7 | §5-4 | 印を保存時に書くので「移行の限界」がある | **読み込みは `localStorage` を保存し直さない** ⇒ ユーザーが保存するまで元データは無傷 = 限界は思ったより軽い |
| 8 | §5-6 | 変異 `slice5` は 2 箇所なので唯一語が要る | 本体を `.slice(0, keepCap);` へ言い直したので **1 箇所**になった |
| 9 | §2-7 | 本チケットは `apEquippedIdsFor` の**中**を触る | **1 バイトも触らなかった**(`getClericSlotsTV` を通るので (1d) は自動で効く) |
| 10 | §8 (0a) | `spelloff` の**判定**がちょうど 1 箇所 | 判定は各ファイル 1 箇所だが**語**は tavern 4 / index 2 ⇒ ⛔ 語で数えない |
| 11 | §2-7 / §6 | 印キーが増えるので `localStorage` のキー集合を数える本が動きうる | **数えている本は実在しない**(`verify_save_slots (5z)` は自分で蒔いたキーしか見ない / `(8z1)` は下限) |
| 12 | §2-7 | `verify_pm_drawer_fit` は印で行が折り返すと動きうる | **判定行まで基準と同一**(`.offTag` が inline なので行ボックスが伸びない) |
| 13 | §2-7 | 「既存の検査が掴んでいるもの」の列挙 | **5 つの振る舞いを 1 本も掴んでいない空白地帯**だった(§12-2 (e)) |
| 14 | §8(非退行の手順) | #69 の抽出器を使って突き合わせる | **現存しない**(前セッションの scratchpad ごと消えた)⇒ 凍結指紋から規則を逆算して再実装 |
| 15 | §8 変異表 | `viewonly` の担当は (3a)(3e) | 実測 **7 本**((1d)(2c)(3a)(3a3)(3b)(3c)(3e)) |
| 16 | §8 変異表 | `markonload` の担当は (2a2) | 実測 **3 本**((2a2)(2b3)(4a)) |
| 17 | §8 変異表 | `slice5` の担当は (2b) | 実測 **2 本**((2b)(2b3)) |
| 18 | §8 変異表 | `noprefix` の担当は (4a) | 実測 **5 本**((1c)(1c2)(1d)(2c)(4a)) |
| 19 | §8 変異表 | `offselected` の担当は (1b) | 実測 **2 本**((1b)(1b2)) |
| 20 | §8 変異表 | `switchdead` の担当は (5a)(5b) | 実測 **15 本** |
| 21 | §8 (0b) | 顔ぶれは `openPrep` の前に仕込めばよい | `openPrep` が **`regeneratePartyMembers()` を無条件に**呼ぶ(`tavern.html:6468`)⇒ #54 以後の既定はソロ。受注ナレの間に入れ直す必要がある |
| 22 | §8 (2b) | 12 個の一覧をそのまま仕込めばよい | `sleep` を含めないと **13 個**になり「直っているのに赤」 |
| 23 | §8 (5a) | 撤退腕の期待値は 1 つに決まる | **一覧に `sleep` が在るかで分岐**(12 / 5 と 13 / 6) |
| 24 | §8 (1d) | `apEquippedIdsFor` は職と枠だけで決まる | **`getLevelFromXP(inventory.xp)` を見る** ⇒ xp を仕込まないと候補 2 件 |
| 25 | §8 (4a) | 「除外キーが `dragonfighters.` で始まる」は (4a) で測れる | **自分で蒔いたキーを数えていて変異 `noprefix` が空振り**(⭐ §12-2 (e) が他人の測定器の欠陥として書いた穴と同型) |
...
#### (f) ⭐⭐⭐ 次のチケットに効く一般則(全部このチケットで実測したもの)

1. **同じ関数の中で「切る」と「足す」が前後していると、片方の欠陥を測る受入がもう片方に汚される。**
   ⇒ **期待値は「もう片方を止めた腕で」と書く。**
   実例 = 起草窓は §2-4(読み込みの切り詰め)と §2-3② / §5-4(#54 のスリープ差し込み)を**別々の節で**測ったので、
   「12 個 → 5 個」と書いた。だが両者は `loadSelections` の中で **`slice` の直後に差し込み**という順で並んでおり、
   素の観測は **6 個**。⛔ 「6 に直す」でも「合成された 6 を assert する」でもなく、
   **`?magesleep=0` を掛けた腕(= もう片方を止めた腕)で 5** と書くのが正しい。

2. **「既存 golden が全部緑」は「その振る舞いが守られている」ではない。**
   §12-2 で名指し 13 本が全部緑だったが、緑の**内訳**を 1 行ずつ読むと、本チケットが触る 5 つの振る舞いは
   **誰も測っていない空白地帯**だった(新キーのスロット搭載 / 読み込みの職別切り詰め / 僧侶の行の `自動 N` /
   NPC の空配列 / 酒場側のスリープ差し込み)。
   ⇒ **「既存が守っているから受入から外す」と書く前に、その本が何を測っているかを 1 行読む。**

3. **1 本だけの差はまず比較器を疑う。本物の退行はもっと素直に出る。**
   項目3 で赤く見えた 3 回は全部比較器の欠陥だった。項目5 でも、
   `verify_swamp_fold` の「+1 行」・`driver_mapdef_step2` の「+1 行」・`verify_walk_block` の「-1 行」は
   **すべて抽出規則の穴**だった(総括行の混入 / マーカー直後の空白 / 再掲ブロック)。
   ⇒ **比較器は「変えていないはずの本で差 0」を先に通す。**
   ⚠ ただし **通す母集団を間違えると意味が無い**: 項目3 は 13 本で通したが、母集団 141 本では 10 個の書式を落としていた。
   **自己検証は「これから当てる母集団と同じ広さ」で行う。**

4. **「キーが在る」「呼ばれた」で緑にしない。本番が書いた値まで見る。**
   さらに強い形として —— **他人の測定器の欠陥を指摘した直後こそ、自分の測定器に同じ物差しを当てる。**
   #70 では、`verify_save_slots (5z)` の欠陥(自分で蒔いたキーを同じ `evaluate` 内で数えるだけ)を §12-2 に書いた実装窓が、
   その直後の項目4 で**同じ穴**を開けて変異 `noprefix` を空振りさせた。
   同じ日に起草窓も、実装窓から同じ趣旨の指摘を受けた直後に、自分の依頼書(#71 §2-7)で
   「(1d)(1d2) は反射でも腐らない見込み」という**読みだけの予想**を受入の根拠にしかけた(指摘を受けて `06ec933` で番人を付け直し)。
   **2 窓が独立に同じ穴を開けた** ⇒ これは個人の不注意ではなく、**指摘した側が免疫を持つと錯覚する構造的な型**。

5. **測定器が本番と 1 段でもズレた模型を使うと、答えは静かに別の量になる。**
   実例(起草窓が #71 の下調べで実測)= ライトニングボルトの壁反射で救える不発は、
   **連続角 720 本のレイの近似器では 4 件**だったが、**本番と同じ整数 Bresenham で数え直すと 1 件**。
   近似器は「当たる向き」を**過大評価**する方向へ外れる(連続レイは整数マス列が通らない隙間を通れるため)。
   ⚠ 本チケットの `openPrep` の罠(`regeneratePartyMembers()` を無条件に呼ぶ)も同型だが、
   **酒場を経由する受入にだけ効く**: 既存 `verify_bolt_aim` は `playSeed` で `evaluateOnNewDocument` を仕込んで
   `index.html?autoplay=30&diag=1` へ直接入るため踏まない(起草窓が追試)。項目4 は酒場を経由したので踏んだ。

⭐ 加えて、非退行の実務として本チケットで確かめたもの:
**色が動いた本は「再走 2 回 + 凍結指紋との差 0」で決着できることがある**(#69 は影のツリーが要ったが、#70 は要らなかった)。
⇒ **影のツリーは最初から組まず、まず再走 2 回で切り分ける**。ただし
**「安定した非緑」という分類そのものが実測で崩れる**(上表 30)ので、⛔ 前のチケットの分類を根拠に再走を省かないこと。

#### (g) ⚠ 別件として記録(⛔ 本チケットでは直さない・§11 候補)
...
- **エルフの既定装備が 2 ファイルで割れている。**
  ...
  **習得は揃っていて、既定で装備する 3 つだけが割れている**。⇒ 入り口(酒場 / 本体)によってエルフの初期挙動が変わる。
- **`driver_action_priority --negative` の変異 N3 アンカー腐敗**(#35 `97f350d` 以来)。本チケットでも exit 1 のまま(逐語一致)。
- **`probe_party_size` が自力終了しない**(600 秒で打ち切り)。
- ⚠ **既知フレークの一覧を 7 本へ更新すること**: 既存 5 本 + `driver_speech_engine` + `verify_run_chronicle`。

#### (h) 実機 / 実感の確認(§9)は**ユーザーの宿題**

⛔ 実装窓は実プレイで合否を決めていない(§8 冒頭の方針どおり、受入は決定論の測定だけで組んだ)。
次の 5 点はユーザーが実機で見て決める:
引き出しで「使わない」が 3 種の行すべてで一目で分かるか / 印が暗幕の上で読めるか・iPhone 縦で折り返さないか /
僧侶で除外した呪文を本当に唱えないか・枠の総数が減る違和感 / スリープ 0 が街・ワールドを経由しても 0 のままか /
Lv4 以上の魔法使いで上限いっぱいに配分して酒場を出入りしても減らないか。
⚠ ローカルは **http 起動が必須**(`file://` ではナレ音声が鳴らない)。

#### (i) ポートと後始末

- 使ったポート: 各ドライバが自分の持ち分を使うため、割り当ての 10347〜10349 / 10351〜10363 のうち
  実際に使ったのは **10347**(`auto_debug_run --port 10347`)と **10351〜10363**(`verify_spell_off` の素と変異 12 本)。
- 着手前の LISTEN = **8765**(ユーザーの試遊サーバ `python -m http.server`・PID 19452)/ 9010・9180(Logitech G HUB)。
  **走査後もまったく同じ 3 本のみ・PID も同一**。9000〜11000 帯に残骸 0、`tools/` / `df_` / `verify_` / `driver_` / `probe_` を
  掴む node・python・chrome は **0 本**。⛔ 8765 は落としていない。
- ⇒ **次の新規ドライバの base は 10371 以降**(#71 の起草窓プローブは 10391 帯)。
