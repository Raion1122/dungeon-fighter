...
| 項目 | hash | 内容 |
|---|---|---|
| 項目1 | `f1917cf` | 装置 `tools/verify_cone_cast.js`(§0 (0a)〜(0d) / port 9940)。⚠ 本依頼書 590 行も**このコミットに同梱**(起草時点では未追跡のままだった) |
| 項目2 | `004b5e3` | 円錐探索を共通ヘルパーへ / 8 方向 + 清潔優先の 2 段 / 届かなければ詰め寄る |
| 項目3 | `4c69862` | 負のコントロール **14 本**を実装し `--negative` を全緑(41/41) |
| 項目4 | 本節を書いたコミット | 既存 golden 5 本の非退行 / §12 の実測記録 / 台帳へ #50 完了 |

### 12-2. 受入条件 — `tools/verify_cone_cast.js`(新規 2,021 行)

    node tools/verify_cone_cast.js              -> 19/19 PASSED   FAILED 0   PENDING 0   (exit 0)
    node tools/verify_cone_cast.js --negative   -> 41/41 PASSED   FAILED 0   PENDING 0   (exit 0)

- assert **19 本** / 変異 **14 本**。素は約 3 分 / `--negative` は約 13 分。⚠⚠ **逐次実行必須**。
- ポート: base **9940** / 変異 **9941〜9954**(予約は 9958 まで / 予備 9959)/ 撤退アーム **9960**。
  ⭐ ドライバ側変異 2 本(`noknown` / `flatpop`)は本番バイトを触らないので**ポートを使わない**。
  他ドライバとの衝突 0 本(#49 の 9880 帯には一切触れていない)。

### 12-3. 既存 golden 5 本の非退行(⭐ 1 本ずつ順に実行。headless の並走なし)

⭐ **母集団は「前のチケットが数えた本数」ではなく、締めでもう 1 回自分で grep して数え直した**:

    grep -ln "allyBurningHands\|allyConeOfCold\|burning-hands\|cone-of-cold\|noteAoeOutcome\|__aoeStats\|partyInArea\|coneTiles" tools/*.js
      -> tools/driver_action_priority.js / tools/driver_field_step7.js / tools/verify_cone_cast.js  = 3 本

⇒ 起草時(§8)の **2 本**から 1 本増えているが、増えた 1 本は**本チケットが項目1 で新設した
`tools/verify_cone_cast.js` 自身**(= golden ではない)。⇒ **既存 golden の母集団は 2 本のまま**で
依頼書 §8 の記載と一致。そこへ直近チケットの golden 3 本を足した **5 本**を基準にした。

| golden | 依頼書 §8 の基準 | 締めの実測(2026-09-04) | 判定 |
|---|---|---|---|
| `node tools/driver_action_priority.js` | 92/92 PENDING 0 | **PASSED 92 / FAILED 0 / PENDING 0** | 一致 |
| `node tools/driver_field_step7.js` | (着手前に走らせて記録) | **79/79 PASS**(`(G0)` = golden のキー集合 6/6 一致) | ⭐ 基準を**本チケットで新規に記録** |
| `node tools/driver_cast_circle.js` | 56/56 | **56/56 PASS** | 一致 |
| `node tools/verify_roll_target.js` | 30/30(`--negative` 53/53) | **30/30 PASSED FAILED 0 PENDING 0**(`--negative` **53/53 PASSED FAILED 0 PENDING 0**) | 一致 |
| `node tools/verify_enemy_name_label.js` | 30/30 | **30/30 PASSED FAILED 0 PENDING 0** | 一致 |

- ⭐ **5 本とも 1 回目で緑**。単発の赤による再実行は **0 件**。⚠ 実プレイ系なので **1 本ずつ逐次**で回した。
- ⛔ **golden の期待値は 1 文字も書き換えていない** — `git diff --numstat 739a9e4 HEAD` に
  `tools/driver_action_priority.js` / `tools/driver_field_step7.js` / `tools/driver_cast_circle.js` /
  `tools/verify_roll_target.js` / `tools/verify_enemy_name_label.js` は **1 本も出てこない**。
- ⚠⚠ `driver_field_step7.js` の **(P2) は cone 80.5% のまま**。§2-6 のとおりこれは**鏡**なので
  **変わらないのが正しい**。⛔ この 80.5% を本チケットの成果として引用しないこと(→ 12-5 (10))。
- ⚠ baseline worktree(`%TEMP%\df_step7_baseline`)は締めの時点では**健全**だった
  (`index.html` 1,402,889 bytes を保持)。壊れて残っていたら
  `git worktree remove --force <dir>` で作り直す(→ 12-5 (11))。

### 12-4. 変更範囲の実測(`git diff --numstat 739a9e4 HEAD` = 項目 1〜3 ぶん / 本節を書いたコミットの直前)

     114     65   index.html                  ← 本体(ヘルパー新設 + 2 呪文の差し替え + STEP3 + 撤退スイッチ + 検証シーム)
       1      1   tavern.html                 ← changelog 1 行(§10 の文面のまま)
    2021      0   tools/verify_cone_cast.js   ← 新規
     590      0   実装依頼書/2026-09-04_cone-spell-cast-rate.md  ← 本依頼書(項目1 に同梱)

⛔ **`mageAI` の梯子 / `RANGE` テーブル / `tavern.html` の説明文は 1 行も触っていない**(§3 の宣言どおり)。

---

## ⭐ 12-5. 依頼書が外していた点(実測で崩れた 11 件)

### (1) 行番号は 12 件すべて一致した(ズレ 0)

着手時の HEAD が起草基準の `739a9e4` と**同一**だったため、§2-1 の呼び口 12 件は
行番号まで全部当たった。⭐ #6 の「8 件中 8 件ズレ」とは**逆のケース**。
⚠ それでも「一致していた」は結果論なので、次のチケットでも着手前の突き合わせは省かないこと。

### (2) ⚠ §2-1 の「1 文字違わぬ写し」は**誤り**

`allyBurningHands` と `allyConeOfCold` の探索ブロックを `diff` に掛けると **1 行差**があった —
`allyConeOfCold` 側にだけ

    // 円錐内のタイル一覧 (方向 d / 距離 step / 横ずれ lat に対応)

のコメントが 1 行多い。⇒ **単一の文字列で両方を掴めない**ので、STEP2 の置換は
**2 回に分けた**(片方ずつ、それぞれ出現数ちょうど 1 を検算しながら)。

### (3) ⭐⭐⭐ §4 の ⚠⚠ 警告そのものが誤りだった(変異 `nobreak`)

依頼書 §4 は「清潔パスの `break` を消すと、清潔パスと味方入りパスで `cnt` の値がずれる」と
警告し、それを変異 `nobreak` で機械的に守れと指示していた。**実測すると完全な no-op**。

    for (const t of tiles) {
      if (partyInArea(...)) { party++; if (pass === 0) { rejected = true; break; } }
      cnt += enemiesInArea(...).length;
    }
    if (rejected) continue;

- `break` だけ落としても `rejected` は true のままなので、直後の `if (rejected) continue;` で
  **その方向は丸ごと捨てられる** = 途中まで足した `cnt` は**どちらにせよ 1 度も使われない**。
- しかも味方入りパス(pass 1)は `rejected` を立てないので、**元から全タイルの合計**。
- ⇒ **`break` は純粋な最適化で、消しても振る舞いは 1 ビットも変わらない。**

⇒ 変異は #38/#43/#45 の作法どおり**作り替えた** — 「短絡**も棄却も**落として `cnt` を
最後まで足し、その方向を採る」形にすると、撤退アームで旧探索の鏡と **142 件**食い違い
(1a) が赤くなる。⛔ assert は 1 文字も緩めていない。

### (4) ⭐⭐ 空撃ちのガードは 2 本ある(変異 `zerofoe`)

依頼書は「`if (cnt <= 0) continue;` を消す」と指定したが、**それだけでは無害**。
採用条件 `if (cnt > bestCount)` の `bestCount` 初期値が **0** なので、`0 > 0` は偽 =
敵 0 体の方向は**どのみち `best` にならない**。
⇒ 変異は `best` への昇格まで書く形へ作り替えた。「どの方向にも敵が居ない標本」
**109 件**で (2c) が赤くなる。

### (5) ⭐⭐⭐ `only4dir` は担当節 (2a) では**原理的に捕まらない** → 担当節を (2b) へ移した

実測(合成盤面 240 件):

| 探索 | 発射率 | 撤退アーム比 |
|---|---|---|
| 素(8 方向・2 段) | 212/240 = **88.3%** | 3.71 倍 |
| 変異 `only4dir`(4 方向・2 段) | 184/240 = **76.7%** | **3.23 倍** ← (2a) の敷居 3.00 倍を割らない |
| 撤退(4 方向・拒否権が絶対) | 57/240 = 23.8% | — |

⭐ 理由 = **拒否権の寄与が斜めの寄与を圧倒する**。斜めを消して落ちる幅(88.3% → 76.7%)は
3 倍の敷居をまたげない。
(⚠ 表の「3.71 倍」は 212/57 = 3.719 を装置が切り捨て表示したもの。12-6 の **3.72 倍**と同じ値。)
⚠ **盤面のせいではない** — 依頼書 §2-5 の**実プレイ値でも同じ**(拒否権だけ外す 51.9% /
現行 5.6% = **9.3 倍**)。

⇒ 担当節を **(2b)** へ移した。根拠 = ドライバの独立実装 `hasCleanDir` は **8 方向**で
清潔な方向を探すので、「清潔な方向が斜めにしか無い」標本(実測 131 − 57 = **74 件**)では
4 方向の清潔パスが必ず空振りし、pass 1 で味方入りを掴む = (2b) が赤くなる。
⭐ (2a) は `record` に残したので、**3.23 倍という空振りの実測値が毎回ログに出る**。

⚠⚠ **副産物の知見**: (2a) の「3 倍以上」は結局 **`vetoback` しか捕まえられない弱い敷居**
だと分かった(`vetoback` 単独は 2.30 倍で赤)。強めるなら「素のアームで斜めが選ばれた
件数 >= N」を足すのが筋だが、**assert の強化は本チケットの範囲外**なので別チケットへ。

### (6) ⚠⚠⚠ (3c) の測定点を移した

依頼書 §8 (3c) の「対象が**隣接 1 マス**なのに円錐へ入らない盤面」は、**8 方向では
原理的に作れない** — 隣接 8 マスは必ずどれかの円錐の step1 に入る(円錐の被覆は
chebyshev ≤ 3 を全部覆う)。
⇒ (3c) を **「円錐の実効射程内 0〜3 マスでは 1 件も詰め寄らない」+「円錐が立たない
距離 0(術者と同じマス)では `allyBasicAttack` へ落ちる」**へ移した。
変異 `advadjacent` は距離 0 の盤面で必ず赤くなるので**検出力は落ちていない**。

### (7) ⚠⚠⚠ 「距離 4〜8 なら円錐の外」は**偽**

斜め方向の step3 lat=±2 は `(5, ±1)` = **chebyshev 5** まで届く。
⇒ (3a)(3b) の盤面は、ドライバ側の独立な幾何 `coneSet` で「**円錐に入らないオフセット**」を
先に作ってから選ぶ形にした。
`reachdrift`(`coneTilesFrom` の上限だけ 4 に直書き)はこの `coneSet` を広げないので、
(3a) の距離 4 盤面が「詠唱した」= ADV も BASIC も呼ばれず赤くなる。
⇒ 担当節は **(3a) 単独**にした(依頼書の「(3a) or (3c)」を両方 `targets` に書くと
(3c) が緑のままで変異が落ちる)。

### (8) ⭐ 合成盤面は (2a) が成立するように作り直した

項目 1 の初版(術者 1〜3 マス / 前衛 1〜3 人)は**旧探索が 52.5% も撃ててしまい**、
新 100% との比が **1.9 倍**で (2a) の 3 倍に原理的に届かなかった。
⚠⚠ 依頼書 §8 (0b) は「差 50 件以上」しか要求していないので、**(0b) は緑のまま (2a) が死ぬ**
構造だった。
⇒ 起草時の実測(§2-5 の距離分布 = 18/29 が 4 マス以上 / 前衛 3 人編成)に合わせて
**術者 1〜5 マス・前衛 2〜4 人**へ修正 → 旧 23.8% / 新 88.3% = **3.72 倍**、
差の出る標本 **155 件**。

⭐⭐ **一般化 = 母集団ガードは「差が出ること」だけでなく、
「後続の assert の敷居を満たす差が出ること」まで縛らないと、母集団が緑のまま本体が死ぬ。**

### (9) ⚠ 仕込みは **3 点セット**が要る(依頼書は 2 つしか書いていない)

- `localStorage['dragonfighters.knownSpells']`
- `localStorage['dragonfighters.partySkills']` — ⚠ **配列**で渡す
- ⭐ **`sessionStorage['dragonfighters.partyMembers']` も必須**(依頼書に記載なし)。
  無いと `buildParty` がランダムに組み、**魔法使いが 1 人も来ない run** が出る。
- ⚠ Lv3 の mage 呪文スロット上限は **5**。`burning-hands ×4` + `magic-missile ×1` で
  ちょうど収まる(超えると**黙って切られる**)。
- ⛔ `noteAoeOutcome` のキーは **`skill.name` = `"バーニングハンズ"`(日本語)**。
  `"burning-hands"` ではない。

### (10) ⛔ §2-6 の鏡は今回も触っていない

`tools/driver_field_step7.js:330-347` の `conePick` は **旧アルゴリズムのままの鏡**である。
⇒ その (P) の cone 行(**80.5%**)を「本チケットの修正後の実測値」として引用してはいけない。
**実測値の正は `tools/verify_cone_cast.js` だけ。**
(§11 の「壊れていないものを触らない」に従い、鏡は追随させていない。)

### (11) ⚠ `driver_field_step7.js` の baseline worktree が壊れて残ることがある

`%TEMP%\df_step7_baseline` が `index.html` を失った状態で残り、`git worktree add` が
`already exists` で落ちる。**`git worktree remove --force <dir>` で作り直せば通る**(79/79 PASS)。

---

### 12-6. 効果の実測

| 測り方 | 旧(= `?conecast=0`) | 新 | 倍率 |
|---|---|---|---|
| 合成盤面 240 件の発射率 | 57/240 = **23.8%** | 212/240 = **88.3%** | **3.72 倍** |
| 実プレイ 90 秒・廃坑(項目1 時点) | `attempts 8 / cast 0` | — | 報告された症状が**ドライバ内で再現**した |
| 実プレイ 90 秒・廃坑(項目2 実装後) | — | `attempts 7 / cast 6` | — |
| 実プレイ 90 秒・廃坑(**締めの走**) | — | `attempts 8 / cast 5` | ⚠ 実プレイは非決定論。**率は合成盤面で測る**(§8 の方針どおり) |

- **(4b)** = 同じ合成盤面で **バーニングハンズ 87.5% / コーンオブコールド 87.5%**
  (完全一致)= **探索が 1 本のヘルパーに寄っていることの機械的証明**。
  ⭐ 変異 `coldstale`(コーンオブコールドだけ旧探索へ戻す)を当てると 87.5% / **26.7%** に割れる。

### 12-7. 装置の恒久メモ(同型チケットで再利用できる)

- ⭐ **`replaceInFn(body, 関数の見出し, from, to)` を新設**した。
  逐語置換の起動時検算(**出現数ちょうど 1**)を通せない当て先
  (`allyBurningHands` と `allyConeOfCold` の双子)を狙い撃つ手段。
- ⭐ **各変異の `record` に `(0a)` を入れる型**を採用した。
  変異ページが構文エラーで起動しないと**全 assert が赤になり負のコントロールが偽の緑になる**が、
  `(0a)`(`window.__cone` が取れるか)の記録行を見れば**一発で判別できる**。
- ⚠⚠ **実プレイ系は逐次**。`--negative` 1 走 ≒ 13 分 / 素 ≒ 3 分。

### 12-8. 残っている宿題(§9 の実機体感 — **未了**)

1. 廃坑を魔法使い入りで 1 周し、バーニングハンズが実際に何度も出るか。
2. ⭐⭐ **炎が味方を包む絵面が許せるか**(実測 = 味方 1 人が **71%** / 2 人が **14%**)。
   許せないなら「清潔な方向が無いときは 1 人までなら許す」等の上限を後から足す。
3. 魔法使いが**前に出すぎていないか**(STEP3 の「詰め寄る」の副作用)。
4. ⚠⚠ **斜め方向は `spawnConeFlames` / `spawnGroundFx` / `spawnFireImpactBurst` が
   初めて通る経路**。絵が横に伸びる・向きがおかしいなら別チケットへ。
5. コーンオブコールド(Lv5〜)でも同じ改善が体感できるか。
