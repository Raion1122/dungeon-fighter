...
### 14-10. STEP4(項目4)の実測 — 2026-09-08

**実装**(`tavern.html` のみ・**+54 / −0**。⭐ 既存行を 1 行も書き換えていない = 純粋な挿入):

- `.patronLabel.promised` = **色だけ**(文字 `#ffe7b4` / 背景 `rgba(12,8,3,0.92)` / 枠 `#e0b43c`)。
  ⛔ `font-size` / `padding` / `top` には 1 文字も触っていない(#57 の実機体感 ① が未消化なので
  動かす余地を残す)。⛔ glow / box-shadow も足していない(#57 の `--holdGlow` が実機未確認)。
  ⚠ `--pm-gold` はマッチング演出の器スコープの変数なので札からは読めない ⇒ 同じ金を直値で書いた
  (⛔ `:root` へ昇格させると影響範囲が跳ねる)。
- 付け外しは `patronLabelPaint()` の **1 行**
  (`lb.classList.toggle("promised", promised && PROMISE_MARK_ON)`)。⭐ 初期描画(`initNpcCrowd`)も
  付け外し(`refreshPatronLabelFor`)も必ずここを通るので **2 箇所目の付け口を作っていない**。
  ⛔ 札を出す席は 1 つも増やしていない(卓の 4 席のまま)。
- カードの出自 `.pmOrigin`(「🤝 酒場で声を掛けた」)。器は `buildPmColumn` で作り、`fill()` で
  `!m.isHero && DFRecruits.has(m.name)` のときだけ `hidden` を外す。
  ⚠ 判定は名前引きなので同名の別人が名簿に居ると両方に付く ⇒ 卓 4 席・上限 3 人ゆえ **許容**
  (⛔ 名簿の同一性システムは作らない = 依頼書 §8-2 / §13)。
- 撤退 `?promisemark=0` = `PROMISE_MARK_ON`(判定 1 箇所)。OFF では色分けが付かず `.pmOrigin` は
  **要素ごと作られない**。⛔ 🤝 の前置は #57 の出荷済み分なので **残す**
  (消すと #57 を撤退させたことになる)。

**受入 `tools/verify_party_promises.js`**: 27 → **35 assert**
((4a)(4a2)(4a3)(4b)(4b2)(4b3)(4c)(4z) を追加)。素 **35/35 PASSED / FAILED 0 / PENDING 0**。

### 14-11. 依頼書 / 指示が崩れた点(項目4。通し番号は §14-1 / §14-4 / §14-7 の 16 件から続く)

17. ⚠⚠⚠ **受入 (4b) の「他は不変」をバッジの数字で測ると、実装は正しいのに赤くなる。**
    バッジは **パネルを開いた時点**でしか名簿を引き直さない(§14-7 の 6 が自分で決めた設計)。
    ドライバが保管庫を直接焼いただけでは表示が古いまま ⇒ 初回実走で
    `badge="🤝 0 / 3"` vs 名簿 2 人 で (4b) が赤くなった。
    ⇒ **本番と同じ口(バッジを押す)を通してから読む**形へ直した。
    ⭐⭐⭐ **一般形 = 「引き直しの契機を持つ表示」を非退行の指標に使うときは、その契機を必ず通す。**
    ⛔ 古い表示を実装の欠陥と読むと、直さなくていい物を直しにいく。
    ⭐ #58 の「門番の下流で測るな」の **時間軸版**(場所ではなく **タイミング** の門番)。

18. ⭐⭐⭐ **依頼書 §8-1「枠を金・背景を濃く」は、class の有無だけを見る assert では緑になる。**
    `.promised` を付けても CSS が効いていなければ見た目は 1px も変わらない。⇒ (4a2) で
    `getComputedStyle` を読み、**素の札と色が違う**ことを実測した(金枠 `rgb(224,180,60)` /
    `rgba(12,8,3,0.92)` vs 素 `rgba(160,124,60,0.75)` / `rgba(24,18,10,0.78)`)。
    ⭐ #57 の「言い分けは文字だけでなく VFX(数値 assert では原理的に捕まらない)」の同型。

19. ⭐⭐ **「文字サイズは決め打ちしない」は「assert を 1 本も置かない」ではない。**
    値を縛らずに書ける不変条件が実在する = 「`.promised` が font-size を **動かしていない**」
    (金枠の札と素の札で `fontSize` が同値。実測はどちらも `12px` だが assert は **同値性だけ**)。
    ⇒ #57 の実機体感 ① で 12px を動かしても (4a2) は赤くならない。
    ⭐ **一般形 = 「測らないと決めた量」でも「この変更がその量を動かさない」は測れる。**
    §2-13 の「golden の空白地帯」を、値を凍結せずに部分的に塞ぐ手。

20. ⚠ **依頼書 §9 の受入条件に「撤退 3 本の独立」と「3 本 OFF でも潜れる」が無い。**
    §12 の表は 3 本を並べているのに、受入は (1d)(3e)(4b) の **各 1 本ずつ**しか見ていない。
    ⇒ (4b3)(`?promises=0` でも色分けは残る)と (4z)(3 本 OFF でも `departToScenario` が 1 回)を新設。
    ⭐ 撤退スイッチの最大の事故は「外したら進行不能」で、それは 1 本ずつの腕では **原理的に出ない**。

21. ⭐ **カードの出自を「`hidden` の器」にしたので、`?recruittalk=0` の腕には 1px も影響しない。**
    母集団 16 本のうち `driver_equip_compact_ios` / `driver_party_view_reopen` /
    `verify_mercenary_roster` / `verify_party_match_setup` / `verify_pm_drawer_fit` は URL に
    `?recruittalk=0` を持ち、そこでは `DFRecruits.has()` が常に false = 出自が 1 枚も出ない。
    ⇒ #56 の「カードの縦が伸びる」前科を、**器を置く場所ではなく `hidden` の使い方**で回避した。

22. ⭐ **項目3 の申し送り「保管庫から読んだ写しと席の本人は別物」は STEP4 では踏まない。**
    `.promised` の判定は `DFRecruits.has(m.name)` = **名前引き**で同一性を要求しないため。
    ⇒ `promisesSeatMemberByName()` は STEP4 では 1 度も要らなかった(申し送りの読みどおり)。

23. ⚠ **§2-9 が確保した変異用ポート「10162〜10169」(8 本ぶん)では 1 つ足りない。**
    変異が 9 本(§10 の 8 本 + 項目2 が新設した `m5w`)になったので、子プロセスは
    **10162〜10170**。⭐ ポートは base ではなく **`--negative` で開くレンジ**で数えること
    (#48 で 3 例目として記録された罠の 4 例目)。

24. ⭐⭐ **新設した 3 つの腕は、既存の変異にも反応した**(実走で確定。⛔ 机上の担当表では書けない):
    `m6`(戻るを確定前から出す)→ 担当の (1e)(1e2)(1d) に加えて **(4z)** /
    `m8`(バッジを引き算で作る)→ 担当の (3d) に加えて **(4b)** /
    `m4`(卓の 4 席以外にも札)→ 担当の (4a) に加えて **(4b)(4b3)(4z)**。
    ⭐ これは冗長ではなく **設計の裏付け** —— 「札は卓の 4 席ぶん」「戻るの口は猶予明けだけ」
    「バッジの数字は名簿の実体」は **撤退スイッチの状態に依らない**不変条件なので、
    どの腕で測っても同じ答えが返るのが正しい。
    ⭐ 逆に言うと **腕を足すと変異の検出力は勝手に上がる** = #59 の「変異表は担当ラベルではなく
    実装の本数で数え直す」の裏返し(**受入の腕**の本数でも検出力は増える)。

### 14-12. 残り = 実機体感(項目4 で増えたぶん)

13. `.promised` の金枠が **12px の札でも判別できる**か(⭐ #57 の実機体感 ① と **同時に**見る。
    機械では「色が素の札と違う」までしか測っていない)。
14. カードの「🤝 酒場で声を掛けた」が **カードを縦に伸ばしすぎていない**か(#56 の前科。
    ⭐ 機械では「`?recruittalk=0` の腕では 1px も動かない」までしか測っていない)。
15. 金枠と #57 の 🤝 前置が **二重に主張して煩くない**か(色と文字で同じことを 2 度言っている)。
16. 上限まで(3 人)約束した日に、卓の 4 席のうち 3 枚が金枠になって **卓が金色に見えすぎない**か。

### 14-13. ★ 締め — #60 の最終結果(項目4 / 2026-09-08)

**受入 `tools/verify_party_promises.js`(base 10161)**

- 素: **35/35 PASSED   FAILED 0   PENDING 0**
- `--negative`: **9 本すべて空振り 0**。担当表は机上ではなく **1 本ずつの実走**で採った:

| 変異 | 何を壊すか | 実際に赤くなった節 |
|---|---|---|
| `m1` | 早期 return の 2 本を `"back"` にする | (1c2),(1c3) |
| `m3` | `resolve()` を無記名へ戻す | (1b),(3a) |
| `m5` | 解散で `DFRecruits.clear()` を呼ばない | (2a) |
| `m5w` | 受注ナレのボタン名を写経へ戻す | (2d) |
| `m6` | 戻るの口を確定前から出す | (1e),(1e2),(1d),(4z) |
| `m2` | 再入場を `openPrep` 経由へ戻す | (3a) |
| `m7` | 「編成を見る」に `btnPartyView` の id を使う | (3b),(3z),(3a) |
| `m8` | バッジを `RECRUIT_MAX` からの引き算で作る | (3d),(4b) |
| `m4` | 卓の 4 席以外の NPC にも札を出す | (4a),(4b3),(4b),(4z) |

- **ポート**: base **10161** / 変異の子 **10162〜10170**(9 本)。
  ⭐ 次の新規ドライバは **10181 以降**。⛔ 10080 は Chrome の `ERR_UNSAFE_PORT`。

**母集団 16 本の非退行**(直列実走・項目3 の実測ログと **集計行 / FAIL 行の集合 / exit** を逐語比較):

| ドライバ | exit | 集計行(逐語) | 項目3 の記録と一致 |
|---|---|---|---|
| `driver_action_priority` | 0 | `[driver] RESULT: PASSED 92 / FAILED 0 / PENDING 0` | ✅ 一致 |
| `driver_depart_menu_clean` | 0 | `══════════ 結果: 41/41 PASS ══════════` | ✅ 一致 |
| `driver_equip_compact_ios` | 0 | `=== WORKTREE: 31/31 PASS ===` | ✅ 一致 |
| `driver_party_view_reopen` | 0 | `========== 結果: 35/35 PASSED / 0 FAILED / 0 PENDING ==========` | ✅ 一致 |
| `verify_darkvision` | 0 | `25/25 PASSED   FAILED 0   **PENDING** 0` | ✅ 一致 |
| `verify_hold_person` | 0 | `31/31 PASSED   FAILED 0   PENDING 0` | ✅ 一致 |
| `verify_party_match_setup` | 0 | `[driver] RESULT: PASSED 36 / FAILED 0 / PENDING 0   (合計 36)` | ✅ 一致 |
| `verify_pm_drawer_fit` | 0 | `75/79 PASSED   FAILED 0   PENDING 4` | ✅ 一致 |
| `verify_prep_retire` | 0 | `30/30 PASSED   FAILED 0   PENDING 0` | ✅ 一致 |
| `verify_quest_walk` | 0 | `25/25 PASSED   FAILED 0   **PENDING** 0` | ✅ 一致 |
| `verify_recruit_size` | 0 | `══════════ 結果: 82/82 PASS ══════════` | ✅ 一致 |
| `verify_recruit_talk` | 0 | `25/25 PASSED   FAILED 0   **PENDING** 0` | ✅ 一致 |
| `verify_mercenary_roster` | 0 | `[mercenary-roster] 44 PASSED / 0 FAILED / 0 PENDING  (44/44)` | ✅ 一致 |
| `verify_run_chronicle` | **0(単独で再走)** | `[run-chronicle] 73 PASSED / 0 FAILED / 0 PENDING` | ✅ 一致 |
| `verify_npc_crowd` | 0 | `32/32 PASSED   FAILED 0   **PENDING** 0` | ✅ 一致 |
| `verify_tavern_map` | 0 | `43/43 PASSED   FAILED 0   **PENDING** 0` | ✅ 一致 |

⭐ **16 / 16 本**が exit・集計行・FAIL 行の集合すべてで項目3 の実測と一致。

⚠ **1 度だけ偽の赤が出た** —— `verify_run_chronicle` の初回実走が
`(2z3)`(母集団ガード「窓の間に敵の HP が実際に減っている」= `boardEnemyLoss=0`)と
その下流の `(2a)` の **2 本**で落ちた(`71 PASSED / 2 FAILED`)。
⭐ **単独で再走させたら `73 PASSED / 0 FAILED` へ復帰**(exit 0)= #55 の分類でいう **型2(偽の赤)**。
真因はオートプレイの観測窓依存(memory の「オートプレイ系ドライバは観測窓依存でフレークする —
2 回走らせてから騒ぐ」)。⛔ 本チケットは `index.html` を 1 度も開いていないので因果が無い。

**撤退スイッチ 3 本**(§12):

| スイッチ | 消える物 | 残る物(= 独立の証拠) | 測っている節 |
|---|---|---|---|
| `?pmback=0` | 「🍺 酒場へ戻る」 | 出発の口・解散の口 | (1d) |
| `?promises=0` | 常設バッジ / 一覧パネル | **頭上札の金枠**・カードの出自 | (3e) + **(4b3)** |
| `?promisemark=0` | 頭上札の金枠 / カードの出自 | **#57 の 🤝 前置**・バッジ / パネル | **(4b)(4b2)** |

⭐⭐⭐ **3 本すべて OFF でも潜れる**((4z)。`?pmback=0&promises=0&promisemark=0` で
`departToScenario()` が **1 回**呼ばれ、追加物は 3 種とも消えている)。
⚠ 撤退スイッチ最大の事故は「外したら進行不能」で、**1 本ずつの腕では原理的に出ない**。

**依頼書の主張が実測で崩れた点**: 通算 **24 件**
(§14-1 の 5 件 + §14-4 の 6 件 + §14-7 の 5 件 + §14-11 の 8 件)。

**コミット**: `28e9643`(STEP1)/ `5dbf35d`(STEP2)/ `bd98552`(STEP3)/ `fe67c14`(STEP4)/ 本行を書いたコミット(締め)。
⚠ **未 push**(push はユーザー承認事項)。
