...
### 12-5. 母集団の非退行(項目5) — 2026-09-25 / 基準 `20323ce`(実装 = `d80ca9b` + `2f31ce6`)

⛔ 本番も `tools/` も 1 バイトも触っていない(走査中も作業ツリー clean)。実装の差分は
`git diff --stat fd67cfe 20323ce -- index.html tavern.html audio.js js` = **`audio.js` 31 行 + `tavern.html` 2 行(changelog)だけ**。

#### (1) 走った腕 = **151 腕**(着手前と同じ腕・同じ順)

- 腕の並びは `baseline_frozen.tsv` から導出し、`armlist73.json` と**順序まで一致**することを assert してから走らせた(⛔ 本数を定数で焼いていない)。
- 走行器 = 項目1b の `sweep73.py` を**そのまま import** し、出力先だけ `%TEMP%\df_pen73\after.tsv` / `after\` / `fp_after\` へ向けた
  (`scratchpad\item5\after73.py`・⛔ 凍結データは上書きしていない)。直列・毎腕後に `df_*` の居残り Chrome を掃除(**全 151 腕で 0**)。
- 所要 = **272.1 分**(腕の秒数の合計。着手前 298.1 分 → **−26.0 分** / 1 腕 **108.1 秒**)。
  縮んだ主因は語りを通る本(`verify_party_promises` 249→39 秒 / `verify_quest_walk` 212→101 / `verify_prep_retire` 147→54 /
  `driver_depart_menu_clean` 126→52 / `driver_action_priority` 122→65)と、非決定な長尺の 2 本(`driver_field_step6` 2,101→1,598 / `auto_debug_run` 1,102→890)。
- 打ち切り = `probe_party_size`(素)の 600 秒だけ(着手前と同じ扱い)。

⚠⚠ **1 回目の起動は 4 腕目で止めてやり直した。** `auto_debug_run.js` は**自前のサーバを 8765 で立てる**ので、ユーザーの試遊サーバ
(`py -m http.server 8765`)が動いていると **1.0 秒で `EADDRINUSE` → exit 1(偽の赤)**になる。
着手前の走査は 8765 が空いた状態で取られていた。⇒ 試遊サーバを止め、その行を捨てて同じ順で再開し、走査後に立て直した
(配信された `audio.js` に `get("penvoice")` が載っていることを確認済)。
...
|---|---|---|
| 緑→緑 | **131** | — |
| 赤→赤 | **15** | `driver_field_step6` / `driver_grid_p4` / `driver_grid_p8` / `driver_mapeditor` / `driver_mapeditor_painting` / `driver_monsters_umberhulk` / `driver_sce1_events` / `driver_speech_v2` / `probe_bandit_map` / `probe_party_size`(素)/ `probe_s2_fold` / `probe_swamp_map` / `sweep_recruit_balance` / `verify_walk_block` / `probe_party_size --negative` |
| **緑→赤** | **2** | `driver_monsters_hobgoblin` / `probe_n4_stall` ⇒ 下の (4) で**どちらも #73 に帰属しない**と決着 |
| 赤→緑 | **3** | `driver_field_verge_gap` / `driver_monsters_griffon` / `driver_speech_engine`(3 本とも §12-0b (4) の「色が動いた本」) |
| exit の値だけ変化 | 0 | — |

- 着手前の赤 18 腕 = 赤→赤 15 + 赤→緑 3。赤→赤 15 腕のうち指紋を比べられる 9 腕は **経路①②とも凍結と一致** =
  「赤のまま・同じ指紋」。残り 6 腕は判定行 0 行(`driver_grid_p4` / `probe_bandit_map` / `probe_s2_fold` / `probe_swamp_map` は exit=3 のまま =
  本自身の早期終了 / `sweep_recruit_balance` / `probe_party_size` 素は同じ exit)か、指紋が動く本(`driver_field_step6`)。
- `probe_party_size`(素 13/7・`--negative` 15/7)は **判定行まで凍結と同一**。⚠ §12-0b (5) のとおりこの本は声を測っていないので、同一は「声が正しい」の根拠ではない。
- 判定行の合計: 着手前 PASS 6,563 / FAIL 33 / PENDING 4 → 実装後 **PASS 6,564 / FAIL 31 / PENDING 4**。

#### (3) 2 経路の突き合わせ

指紋を比べられる腕 = **131**(151 − 判定行 0 行の 16 腕 − 指紋が走行ごとに動く 4 腕。⛔ この 20 腕は exit だけで見た)。

| | 一致 | 不一致 |
|---|---|---|
| 経路① assert id の並び | **130 / 131** | `driver_monsters_hobgoblin` |
| 経路② 判定行の多重集合 | **129 / 131** | `driver_monsters_hobgoblin` / `driver_grid_p5` |

差の分類(⛔ 正規化して消していない。全量は `%TEMP%\df_pen73\fp_after\<arm_id>.json`):

| 腕 | 分類 | 中身 |
|---|---|---|
| `driver_monsters_hobgoblin` | **値差**(id の並びは同一・合否だけ反転) | `(e)` 2 行が PASS→FAIL。観測値は `entries=0` / `minAc=0 maxAc=0` = **装置が 1 件も観測できなかった**(フレークの指紋)。(4) で決着 |
| `driver_grid_p5` | **構造差(経路②のキー)** / 経路①は一致・緑のまま | 判定行の本文が `ally:rogue は …` → `ally:elf は …` の 4 行。本文に**測った仲間の職業**(`'ally:' + a.classKey` = `tools/driver_grid_p5.js:429`)が入り、編成が抽選なので走行ごとに変わる。⚠ **§12-0b の「指紋が動く 4 本」に無かった 5 本目** |
| 指紋を使わない 20 腕 | (比較対象外) | 指紋が動いたのは既知の 4 本(`driver_field_step6` / `driver_field_verge_gap` / `driver_monsters_griffon` / `driver_speech_engine`)だけ。判定行 0 行の 16 腕は空 sha を共有したまま |

#### (4) 色が動いた本の切り分け(再走 2 回。⛔「既知フレーク」の宣言で済ませていない)

| 腕 | 実装後 本走 → 再走 1 → 再走 2 | 凍結 | 判定 |
|---|---|---|---|
| `driver_monsters_hobgoblin` | exit **1 → 0 → 0**(再走 2 回とも 14/14・**経路①②が凍結と完全一致** `b682a20e…` / `42491e26…`) | 0(#72 基準では 1) | **混在 = 非決定**。赤は「観測 0 件」の形。#73 に帰属しない |
| `probe_n4_stall` | exit **1 → 1 → 0** | 0(#72 基準 1) | **混在 = 非決定**。§12-3 (3) で影のツリー(`fd67cfe`)でも 1/1/1 と揺れを実証済み。判定行 0 行の調査プローブ(`?autoplay` で声の経路を通らない)。#73 に帰属しない |

⇒ **2 回とも赤の本は 0 本** ⇒ 影のツリーは要らなかった(`probe_n4_stall` は §12-3 で影でも赤を確認済)。
赤→緑の 3 本は §12-0b (4) の 3 回走行で既に両色に転んでいる本(`driver_field_verge_gap` 1/0/0・`driver_monsters_griffon` 1/1/0・`driver_speech_engine` 1/0/1)。

#### (5) 新規受入(凍結対象外・別枠)

`node tools/verify_pen_narration.js` → **`19/19 PASSED   FAILED 0` / exit 0 / 170.3 秒**(ログ `%TEMP%\df_pen73\item5_accept.txt`)。

#### ⇒ 判定

**151 腕 / 272.1 分で #73 に帰属する緑→赤は 0。** 緑→赤 2 腕はどちらも再走で緑に戻る非決定な本(着手前の一覧どおり)、
指紋の差 2 腕は「観測 0 件」と「本文に抽選の職業名」で説明が付く。成果物 = `%TEMP%\df_pen73\{after.tsv, after\, fp_after\, item5\rr{1,2}\}` /
走行器 `scratchpad\item5\{after73,cmp5,rr5}.py`。

### 12-6. 総括

**何を実装したか(プレイヤー向け)**

1. ダンジョンマスターの**朗読の声(VOICEVOX 69 本)をすべて止めた** — 依頼人の台詞・闇市・判定の声も含め、ゲームから人の声が無くなった。
2. 文字が出るのに合わせて**ペンで紙に書く「シャッ」という音**(合成の帯域雑音・1 回で 3 画)が鳴る。語りはクリックで送れ、放っておいても 2.5 秒で進む。
3. 設定の「ボイス音量」は **「語り 音量」**になり、ペンの音だけを戦闘の効果音とは別に絞れる。

- 撤退 = **`?penvoice=0`**(`audio.js` の `PEN_NARRATION` 1 箇所・逐語 `get("penvoice")` ちょうど 1)。朗読・「ピッ」・`ui` バス・「ボイス音量」が全部戻る。
- 受入 `tools/verify_pen_narration.js`(base 10441 / 変異 10442〜10450)= **19/19**(項目4 で 3 回 + 項目5 で 1 回)/ 負のコントロール **9/9**(空振り 0)。
- 既存 golden の言い直し **0 本**(§12-3)。母集団 **151 腕で #73 に帰属する緑→赤 0**(§12-5)。
- ⚠ **声の信号は新規受入が全部背負う** — 既存 golden で声の変化を検出できる本は 1 本も無い(§12-0b (5))。

**崩れた主張 = 20 件**

| # | 依頼書の主張(節) | 実測 | 出典 |
|---|---|---|---|
| 1 | 朗読の呼び口は **11 箇所**(§2-2) | **16 箇所**(`getVoiceDuration` 3・`preloadVoice` 2 が表に無い)。設計の結論は強まった | §12-0 (2) |
| 2 | 事前読み込みの口は導入の **1 つ**(§2-5) | **3 つ**(`index:39726` / `index:14870` エピローグ / `tavern:9712` 前口上) | §12-0 (2) |
| 3 | 声を止めて変わるのは語りのペースだけ(§1・§3) | `index.html:14539` の **DM バナーの表示尺**も「声の尺 + 600ms」→ 既定 2600ms へ落ちる(書かれていない変化) | §12-0 (2) |
| 4 | §2-1 の引用符付き `narration` の grep で表の 4 行が出る | 返るのは**別の 4 行**(`audio.js:669` / `:717` / `index` / `tavern`)。表の `:231` / `:245` は引用符が無く出ない | §12-0 (2) |
| 5 | 罠1 は「**声の長さぶん**無音で保持」(§2-4) | 放置では**残り 15%**(440 / 919 / 760ms)。**クリックすると保持が爆発**(最悪 10.8 秒) | §12-0 (5-2) |
| 6 | (1d) 「1 文字目まで ≤ 500ms」で罠2 を捕まえる(§8) | localhost では 43 vs 34ms = **永久緑** ⇒「1 文字目までの mp3 要求 0 件」へ | §12-0 (5)・§12-4 (4) |
| 7 | 声に依存する golden は `probe_party_size` 1 本で、§8 はそれを非退行に名指し(§2-11・§8) | この本は声を**測らず無効化**している(`o.voicePatched` はどの assert も読まない) | §12-0b (5) |
| 8 | 名指しの `probe_party_size` は非退行の物差しになる(§8) | **素 / `--negative` とも着手前から赤**(600 秒打ち切り / 15/22・ワールドマップ挿入で導線が腐った) | §12-0b (5) |
| 9 | `driver_bgm_town` の `emptycredit` は VOICEVOX クレジット行の変異(§12-0 (4)) | **BGM の `mine_depths` の credit** の変異。`audio.js:885` をアンカーにする本は 0 本 | §12-0b (6) |
| 10 | 全数走査は **6 時間前後**(§4-3・§8) | 着手前 **4.97 時間** / 実装後 **4.54 時間** | §12-0b (2)・§12-5 (1) |
| 11 | `audio.js` のソースを読む本は **4 本**(§12-0 (4)・§12-0b) | **6 本**(変数引数の `readFileSync` とバイト読みが落ちていた) | §12-3 (1) |
| 12 | 酒場の語りも「クリックで 1 秒以内に次へ」(§8 (1c)) | **前口上(`initTavernPrologue`)には元からクリック送りの口が無い** ⇒ 2.5 秒の固定間隔で見る | §12-4 (4) |
| 13 | 事前読み込みの口はどれも受入で検出できる | **エピローグ `ending_epilogue_*` は manifest に 0 件(未録音)** ⇒ 検出力があるのは 2 口 | §12-4 (4) |
| 14 | (1a) `assets/voice/` への要求を数えれば足りる(§8) | `manifest.json` 自身も `assets/voice/` 配下 ⇒ **mp3 (1a) と manifest (1a') を分けないと `durleak` が分離できない** | §12-4 (4) |
| 15 | 変異表の担当(§8) | **9 本中 6 本が実測と違う**(`manifestleak` / `durleak`「(1c) だけ」→ (1a')(1b)(1c) / `preloadleak` / `blipback` / `sampledpen` / `switchdead`) | §12-4 (3) |
| 16 | (4b) narration 以外は両版で**サンプル単位で一致**(§8) | 自己比較でも最大 5.96e-8 揺れる(Chrome の加算順)⇒ **許容 1e-6** | §12-4 (4) |
| 17 | (2a) 「振幅の山が 3 つ」(§8) | 数え方で答えが変わる(ヒステリシスは 200 種中 6 種で 2)⇒ **プロミネンス併合**で 200/200 が 3 | §12-4 (4) |
| 18 | 既知フレークの一覧(#72 の 9 本) | 本チケットで非決定と実証できたのは **7 本**で集合が違う = **一覧そのものが要約** | §12-0b (4) |
| 19 | 指紋を非退行の根拠にできない腕 = **20**(§12-0b の申し送り) | **`driver_grid_p5` の経路②も動く**(本文に抽選の職業名)= 5 本目 | §12-5 (3) |
| 20 | 全数走査はユーザーの試遊サーバ 8765 と共存できる(項目5 の前提) | `auto_debug_run.js` が **8765 を自前で握る** ⇒ 共存すると 1 秒で偽の赤。止めて走らせ、後で立て直す | §12-5 (1) |

**残 = §9 の実機確認(ユーザー担当・そのまま)**

1. 酒場から依頼を受けてダンジョンへ。**導入の語りで声が鳴らず、文字が出るのに合わせて「シャッ、シャッ」とペンの音がする**か(書いている音に聞こえるか / うるさくないか / 軽すぎないか → 気になれば §5-3 の数値を耳で詰める)。
2. 設定(⚙)を開き、**「語り 音量」でペンの音だけが大きく / 小さくなる**か。戦闘の効果音は変わらないか。
3. 語りが**クリックで送れ、放っておいても 2.5 秒で進む**か(以前は声が鳴り終わるまで進めなかった)。
4. 酒場で依頼人の話を聞く・闇市で買い物をする・判定を振る — **どこでも人の声が鳴らない**か。
5. `index.html?penvoice=0` / `tavern.html?penvoice=0` で、**朗読と「ピッ」と「ボイス音量」が戻る**か。

**別チケット候補(本チケットでは直さない)**: `probe_s2_clear --negative` の `wipeblind` が全滅決着で検査力 0(§12-3 (3))/
`probe_party_size` の両腕がワールドマップ挿入で腐ったまま(§12-0b (5))/ `auto_debug_run.js` のポート 8765 固定が試遊サーバと衝突(§12-5 (1))。
