...
### 14-6. A 側 (STEP1〜3) の着地 — 項目 2・3

| コミット | 内容 |
|---|---|
| `24a7473` | STEP1(拒否権を 3 系統とも `AOE_COVER_ON` 下へ)+ STEP2(`pickAoeBestOrigin` 新設。箱型 5 本の呼び口へ 9 引数目 `AOE_COVER_ON` を追加) |
| `2a785c0` | STEP3(`allySleep` の中心候補を全生存敵へ + `partyCountInArea` / `aoeSafeAllyTag` / `aoeLineSafeAllyTag` で AoE 8 箇所へ「(味方 N 名は無傷)」) |

⭐ **実プレイの巻き込み量**(項目 2 の使い捨て probe / port 10161-10162 の実測):
**素 5.410 体 vs `?aoecover=0` 4.126 体 = +31.1%**、下回った標本 **0 件**。

### 14-7. 受入ドライバ `tools/verify_aoe_coverage.js`(新規・base **10141**)

```
node tools/verify_aoe_coverage.js
  28/28 PASSED   FAILED 0   PENDING 0        (exit 0)
```

| 節 | ラベル | 実測(素 ‖ `?aoecover=0`) |
|---|---|---|
| §0 装置 | (0a)〜(0e) | `AOE_COVER_ON` 素 true / 撤退 false・`__aoeCover` 有 / lane len=**21**(ty=14 tx0=25)・遠クラスタは距離 17 > 射程 10 / 敵 4 体・味方 2 名 / 僧侶 Lv10 に `hold-person` 2 枚 / `__aoeStats.attempts` 両腕とも 1 |
| §1 拒否権 | (1a) 箱型 | `{attempts:1, cast:1, demoted:0, partyRejected:9}` ‖ `{cast:0, demoted:1, partyRejected:9}` |
| | (1b) 実挙動 | 着弾 `(27,12)` 3x3 に**主人公 (29,14) が入っている**(= 味方入りの着弾点が実際に選ばれた) |
| | (1c) 直線 | `cast:1 demoted:0` ‖ `cast:0 demoted:1` |
| | (1d) splash | `cast:1 demoted:0` ‖ `cast:0 demoted:1` |
| §2 巻き込み | (2a) | 5 レイアウトすべてで **理論最大に一致** `A:3/3 B:2/2 C:3/3 D:1/1 E:3/3` |
| | (2b) | **平均 2.40 体 ‖ 0.80 体**(素 `[3,2,3,1,3]` / 撤退 `[1,1,0,1,1]`) |
| | (2c) | 射程外の 3 体クラスタがある盤面でも、選んだ箱の最短距離 **3 ≤ 射程 10** |
| §3 非波及 | (3a)(3b)(3c) | `pickClosestEngagedEnemyFromAlly` = ドライバ独立計算と一致 / mageAI 経由で減った HP は最寄りだけ(群れは 0) |
| §4 スリープ | (4a)(4b) | 素は 2x2 の理論最大 **2 体**(吹き出し「範囲 2体」と一致)‖ 撤退は **1 体** |
| §5 hold-person | (5a)〜(5g) | Lv3 で装備入り 1 枚(撤退では非装備)/ Lv7 で 2 枚・表は turn-undead と完全一致 / 混在盤面で `hold-person` を消費・`turn-undead` は無傷 / アンデッドのみで `pickHoldPersonTarget=-1` → `turn-undead` 消費 / index と tavern の表が一致 / `levelReq` が 2 ファイルとも 3(撤退 index は 5)/ `?holdslots=0` で梯子が #57 着地時へ |
| §6 無傷表示 | (6a)(6b)(6c) | `SPELLファイアボール 範囲 1体(味方 1 名は無傷)` / 直線・splash にも出る ‖ 撤退では 3 系統とも 0 枚 |
| 恒等 | (7a) | pageerror / console error **0 件** |

**⭐ 測り方でとくに効いたもの**(次のチケットへの申し送り):

- **理論最大はドライバが敵の生座標だけから総当たりで出す。** `pickAoeOrigin` の戻り値を母数にすると
  「掃引が取りこぼしたぶんは母数からも落ちる」ので**何体取りこぼしても永久緑**になる
  (#47 の「母数を実装の戻り値から取るな」の AoE 版)。
- **盤面は lane(最長の床の行)の相対オフセットだけで組む。** 絶対タイル座標を 1 つも書いていないので、
  マップが動いた日に「岩盤化して母集団が消える」ことがない。
- **梯子はスロットの消費で判定する**(⛔ `held` で見ると抵抗 RESIST で揺れてフレークする)。

### 14-8. 負のコントロール — **10/10 空振り 0**

```
node tools/verify_aoe_coverage.js --negative
  --negative OK: 10 本すべて担当ラベルが赤くなりました (空振り 0)     (exit 0)
```

| # | 変異 | 点数 | 担当 | 実際に赤くなった |
|---|---|---|---|---|
| m1 | 箱型の拒否権を残す(`pickAoeBestOrigin` への委譲行を落とす) | 23/28 | (1a)(1b)(2a)(2b)(6a) | **一致** |
| m2 | 箱型の狙点を最寄りのまま(敵由来の中心候補を足さない) | 27/28 | (2a) | 一致 |
| m2s | ⭐ **スリープの**狙点を最寄りのまま | 26/28 | (4a) | (4a)(4b) |
| m3 | `pickClosestEngagedEnemyFromAlly` を「最も遠い敵」へ反転 | 25/28 | (3a)(3b)(3c) | **一致** |
| m4 | `hold-person` のスロット表を Lv5 のまま | 25/28 | (5a)(5b)(5e) | **一致** |
| m5 | 梯子を turn-undead の後ろへ戻す(2 行で 1 本) | 27/28 | (5c) | 一致 |
| m6 | ⭐ `tavern.html` の鏡だけ直し忘れる | 27/28 | (5e) | 一致 |
| m7 | splash の拒否権を残す | 27/28 | (1d) | 一致 |
| m8 | 直線の `blocked` を残す | 26/28 | (1c) | (1c)(6b) |
| m9 | 新候補に `aoeBoxReachable` を掛け忘れる | 27/28 | (2c) | 一致 |

### 14-9. 母集団 6 本の非退行 — **§14-0 と完全一致**

exit / 総括行 / FAIL 行の集合が 1 つ残らず §14-0 の基準と同じ(**赤は 0 → 0**)。
...
⚠ 着手前から赤い 2 本(`verify_walk_block` 22/23 / `driver_speech_v2` 45/46)は **#59 とは 1 行も接触しない**ので
回していない(§12 のとおり別チケット送り)。⛔ 緑にしにいっていない。

### 14-10. 依頼書の主張のうち崩れたもの — 通し番号での総括

**(1)〜(6) は §14-2(項目 1 の実測)。(7) 以降が項目 4(受入)で見つけたもの。**

7. ⭐⭐⭐ **§9 §7 の変異表 9 本ではスリープの狙点を 1 本も守れない。**
   `allySleep` の候補拡張は `pickAoeBestOrigin` とは**別実装**なので、M2(箱型の中心候補を消す)を
   入れても §4 は緑のまま。⇒ **10 本目 `m2s` を足した**(実測で (4a)(4b) が赤くなることを確認)。
8. ⭐⭐ **M1 の「呼び口の `AOE_COVER_ON` → `false`」案では 5 本のうち 1 本しか殺せない。**
   ⇒ `pickAoeOrigin` の**委譲行 1 行**を落とす形にした(箱型 5 本すべてが legacy へ落ちる)。
9. ⭐⭐⭐ **M3 の「`const d = Math.hypot(...)` を反転」を比較演算子の反転でやると空振りする。**
   `bestDist = Infinity` から始まるので `d > bestDist` は 1 度も真にならず、返り値は **-1 =「誰も狙わない」**。
   これは「狙点が変わった」ではないので (3b)(3c) が「そもそも撃たない」で落ち、原因が読めない。
   ⇒ **符号を反転**(`-Math.hypot`)して「最も遠い敵を狙う」にして初めて、測っている場所に現れる。
10. ⭐⭐ **§9 §0 の「`__aoeStats` が attempts > 0」は素だけ見ても足りない。**
    (1a) は素と撤退の**対**で主張するので、装置 assert も**両腕**で attempts を確かめる形にした。
11. ⭐⭐⭐ **`window.__aoeCover.pickAoeBestOrigin` を測定に使うと m1 が素通しする。**
    委譲行を消しても新関数自体は生きているため。⇒ 主張の測定は必ず
    **本番と同じ 9 引数の `pickAoeOrigin(..., AOE_COVER_ON)`** か **`ally*` 本体**を通す。
    (シームは「素の値を覗く」用途だけに使い、`partyCountInArea` すらドライバ側で数え直した。)
12. ⚠⚠⚠ **背景タブでは `requestAnimationFrame` が止まるので、`await` される演出を 1 本でも
    取りこぼすと `page.evaluate` が protocolTimeout(240s)までハングする。**
    初回実走で `castMagicMissileBarrage` を取りこぼして実際に踏んだ(`spawnFireballProjectile` /
    `spawnArrow` は潰していた)。⭐ 数え方 = 各 `ally*` 関数の `await <fn>(` を機械的に列挙する
    (実測: この 3 本だけ。残りは `dfPlayCast` / `sleepMs` / `allyAdvanceTowardPoint` / `allyBasicAttack`)。
13. ⭐ **M9(射程外へ撃つ)は「遠いクラスタの方が敵が多い」盤面でないと空振りする。**
    掃引は最大巻き込みを採るので、遠い方が少なければ射程検査を外しても選ばれない。
    ⇒ 近 1 体 vs 遠 3 体(距離 17 > 射程 10)にして初めて (2c) が赤くなる。
14. ⚠ **§9 のポート確保「10141〜10149」では足りない。** 変異が 10 本になったので
    実使用は **10141(素)+ 10142〜10151(変異)**。⭐ 10151 は項目 1 の使い捨て probe と同番だが、
    あの probe は repo に入れていない(実行時に衝突しない)。
15. ⭐ **依頼書 §9 §5 は `CLERIC_SLOTS_TABLE` の突き合わせだけを求めていたが、`levelReq` の鏡も 2 枚ある**
    (§14-2 の (3) で見つかった `index.html` の `CLERIC_SKILLS`)。⇒ (5f) で 2 ファイル同時に縛った。

### 14-11. 残 = 実機体感(⛔ ここから先は数字では決まらない。依頼書 §10 の 6 項目)

1. **仲間ごと巻き込んで撃つ絵が「バグに見えない」か**(「(味方 2 名は無傷)」で伝わるか)
2. **巻き込みが増えた実感があるか**(数字ではなく体感。実測は屋内 +31.1%)
3. ⭐⭐ **難易度が下がりすぎていないか** — シナリオ 3(沼地・素装備 70% で調整済)の勝率
4. **僧侶の金縛りが 1 ランに複数回出るか**(Lv7 以上で 2 枚)
5. **アンデッド戦でターンアンデッドが出続けているか**(§2-9 の構造保証が実プレイでも効くか)
6. ⚠ **#57 の金の輪の実機体感 8 項目がまだ未消化。** 金縛りの頻度が上がるので
   **同時 4 体の描画負荷**が現実問題になる ⇒ 先に #57 の項目 3・5 を見てもらうのが望ましい

### 14-12. ポート台帳(#59)

| 用途 | port |
|---|---|
| 受入 `verify_aoe_coverage`(素) | **10141** |
| 同・変異 m1〜m9 + m2s | **10142〜10151** |
| 項目 1 の使い捨て probe(repo 外) | 10151 |
| 項目 2 の使い捨て probe(repo 外) | 10161 / 10162 |
| 項目 3 の使い捨て probe(repo 外) | 10171 |

⚠ **10161 は隣窓の #60 が取った**(`verify_pm_return`)。
⇒ **次に新規ドライバを立てるときの安全な base は 10181 以降。**
⛔ 10080 は Chrome の `ERR_UNSAFE_PORT`。

### 14-13. changelog / コミット

- 受入(項目 4)は **`tools/` と `実装依頼書/` しか触っていない**ので changelog は**不要**
  (`scripts/hooks/check_changelog.py` のトリガーは `index.html` / `tavern.html` / `audio.js` のみ)。
- 本番 3 ファイルを触ったのは項目 1〜3 で、changelog は §14-5 の 1 行 + 項目 2/3 のぶんが入っている。
