...
✅ **完了(2026-09-10 実装窓 / dev-loop 4 項目・停止 0 回)**。
⛔ **未 push**(`origin/main` = `69d6eed`。#64 の 3 本 + 本節を書いたコミットの計 4 本が未 push)。
...
| hash | 内容 | 差分 |
|---|---|---|
| `f5091a7` | 項目1 — 本番実装 STEP1〜4(`tavern.html` / `world.html` / `town.html`)+ changelog 3 行 | 3 files / **307 挿入 3 削除**(tavern 84 / town 98 / world 128) |
| `2e28625` | 項目3 — 受入 `tools/verify_quest_visibility.js`(新規・base **10221**) | 1 file / **1179 挿入** |
| `247685a` | 項目2 — 既存 golden `tools/verify_npc_crowd.js` の言い直し | 1 file / **154 挿入 7 削除** |
| (本節を書いたコミット) | 項目4 — 台帳 `実装依頼書/README.md` の #64 行 + 本節 | `.md` 2 枚のみ |

⚠ **`git diff --name-only 69d6eed HEAD` は 4 ファイルではなく 5 ファイル。**
§3 の「変更範囲」が **4 枚**(本番 3 + 受入 1)なのは起草時点の見積もりで、実際は
**`tools/verify_npc_crowd.js` が 5 枚目**として加わった(13-6 の 8 番)。
⭐ ただし §3 の本質(⛔ `js/*` / `index.html` / `js/world-map.js` / `js/save-slots.js` を開かない)は守られている
—— 5 枚のどれも `js/` でも `index.html` でもない。

---

### 13-2. 着地した実装の姿

- **器** … localStorage `dragonfighters.activeQuest` = `{id, place, title, at}`。
  `js/save-slots.js` の `KEEP` に入れていないので**スロットごとに独立**(⛔ `js/save-slots.js` は 1 バイトも触っていない)
- **書き口** … `openPrep` の中 **1 箇所**。⭐ 呼び口 4 本すべてがここを通るので、
  **闇市と掲示板の生成クエストにも自動的に乗る**(§2-8 の予想どおり)
- **消し口** … `consumeResult()` の中 **1 箇所**。クリア / 敗北 / 撤退の **3 経路すべて**がここへ来る
- **副行** … 3 面とも既存 `<small>` の `textContent` を差し替え(⛔ 行は増やさない = 帯の高さが動かない)。
  文言は `受注中 — 廃坑「ゴブリンの巣窟」`
- **封蝋** … `.worldSeal`(`.worldSign` の**子**)。出す条件は `questDest`(⛔ 器からではない)
- **戻る道** … `#worldBack` / `#townBack`。`position: fixed` / `z-index: 12` / 実測 **144x44px** /
  左上 `(10, 8 + safe-area)`。遷移は `location.href = "title.html"`(⛔ `replace` にしない = 戻れなくなる)
- **撤退** … `?questmark=0` / `?titleback=0` の **2 本独立**((6c) が機械証明)

---

### 13-3. 受入 `tools/verify_quest_visibility.js`(新規・base 10221)

**最終状態 `247685a` での実走(2026-09-10)**

```
node tools/verify_quest_visibility.js              →  素 39/39 PASSED  (PENDING 0)          exit 0
node tools/verify_quest_visibility.js --negative   →  10 / 10 実行 (PENDING 0)               exit 0
                                                      空振りした変異は無い (PASS)
```

変異 10 本すべてが担当の節だけを赤くした(効きすぎ 0 件):

| 変異 | 赤くなった節 | 緑のまま |
|---|---|---|
| `sealfromvessel` | (3c) | (3a) (3b) (3e) |
| `addline` | (2c) | (2a) |
| `hudid` | (4d) | (4a) |
| `seamwindow` | (5a) | (5c) |
| `lsword` | (5b) | (5a) (5c) |
...
| `noclear` | (1c) | (1a) (1b) |
| `skipgen` | (1b) | (1a) |
| `sealsibling` | (3b) | (3c) (3e) |

- 変異は**配信の差し替え**方式なので、`--negative` を回しても本番ファイルは 1 度も `M` にならない
- 所要 = 素 1 回 ≈ **17 秒** / `--negative` 全 10 変異 ≈ **3 分**
- ⭐ **§9 の表に無い (3e) を新設** … `?questwalk=0` + `questDest` あり → `.worldSeal` **0 枚**。
  13-6 の 3 番(封蝋の判定式)を機械で縛るための節。⛔ **弱めた条件は 0 件**

---

### 13-4. 既存 golden の言い直し — `tools/verify_npc_crowd.js`(32 → 33)

```
node tools/verify_npc_crowd.js              →  33/33 PASSED   FAILED 0   PENDING 0          exit 0
node tools/verify_npc_crowd.js --negative   →  58/58 PASSED   FAILED 0   PENDING 0          exit 0
                                               13 変異すべて実装済・空振り 0
```

⛔ 期待値を 1 つも下げていない。装置 assert **(3c-pick)** を**新設**して 32 → **33** へ**増やした**。
詳細は 13-6 の 8 番・9 番。
...
`tools/*.js` **140 本**のうち `tavern.html` / `world.html` / `town.html` に言及するのは **59 本**。
そのうち **52 本**を実走した。除外 **7 本**の根拠:

- コメントに語があるだけで `goto` しない **6 本** …
  `driver_field_step0` / `driver_field_step1_geo` / `driver_field_step5` / `driver_field_wagon` /
  `driver_mapdef_step1` / `sim_plaza_entry`
- 自前の `__probe_town.html` を配る **1 本** … `probe_town_mask`

⭐ 残り **81 本**は 3 ページに 1 度も言及しないので**原理的に影響を受けない**
(`git diff --name-only 69d6eed HEAD` に `js/*` も `index.html` も 1 バイトも無い)。

**#64 が緑を赤にしたのは `verify_npc_crowd` の (3c) ただ 1 本**(→ 13-6 の 8 番で言い直し)。

**着手前から赤だった 4 本**(いずれも #64 が触らない領域。実装後も**同じ**):

| ドライバ | 着手前 | 実装後 | 赤の理由 |
|---|---|---|---|
| `driver_mapeditor` | EXIT=1 176/179 | 同じ | 地図エディタの実マウス操作 3 件(`map-editor.html`) |
| `sweep_recruit_balance` | EXIT=1 装置崩れ 4/4 | 同じ | `4_partySize(got=1 want=4)`。#61 以降の腐り。研究用スイープ |
| `probe_party_size` | EXIT=124(900 秒打ち切り) | 同じ | 編成人数が全シナリオ 1 人にしか見えない。#61 / #55 以降の腐り |
| `driver_monsters_umberhulk` | EXIT=1 21/22 | 同じ | umber hulk の gaze 再発火 |

⭐ **フレーク 1 本** = `driver_monsters_griffon`(1 回だけ 15/17 → 本番ツリーで再走 2 回とも **17/17**)。
型2 の偽の赤なので**記録に残さない**。

---

### 13-6. ⭐⭐⭐ 依頼書 / 手法が崩れた点(**9 件**)

**1. ⚠⚠⚠ 基準取りの手法そのものに欠陥があった(実装窓が自分で踏んだ)**
`git worktree add --detach` は `core.autocrlf=true`(このリポは `.gitattributes` **無し**)の下で
チェックアウトするため、**本番ツリーに LF で置かれている 389 ファイルが隔離ツリーでは CRLF** になった
(`.js` / `.html` / `.py` / `.json` / `.css` の **211 本**を比べて **149 本**が食い違い)。
実害 = `verify_road_ambush` が **EXIT=3 で 0 秒即死**。変異 `boxleak` のアンカーが `\n` を含むので
CRLF のファイルには 1 件も一致せず、**素の assert が 1 本も走らなかった**。
⇒ これは「着手前から測定器が死んでいた」のではなく**基準取りの手法が作った偽の赤**で、
本番ツリーでは **41/41 exit 0**。
⭐⭐⭐ **一般解 = 隔離ワークツリーで基準を取るときは、作った直後に本番ツリーと byte 比較して行末を揃える。**
揃えないと行末に依存する assert(変異アンカー / 逐語 source assert / 配信バイト数 / sha1)が**全部偽の赤**になる。

**2. ⚠⚠ §9 の母集団 14 本では足りなかった — 実際は 59 本**
さらに 1 回目の grep も痩せていた(**導線の語に `tavern.html` を入れ忘れて 38 本**)。
2 回目で **13 本**追加。#62 の「リスト外で 5 本が赤くなった」と同型。
⭐ **一般解 = 母集団は「変更したファイルを読むかどうか」で引く。機能の語で引くと必ず痩せる。**

**3. ⭐⭐⭐ §6-1 の封蝋の写経コードは、同じ節の要求を満たしていなかった**
§6-1 は「判定式は入場条件と同じ = **印が出る ⟺ 入れる**」と要求しているが、
入場条件の実体は `if (questWalkOff || !questDest) return;` を含む 2 行なのに、
写経コードは `questWalkOff` を見ていない。写経どおりだと `?questwalk=0` のとき
**印は出るのに入れない**(まさに依頼書自身が**罠 B** と呼んだ形)。
⇒ 実装は `!questWalkOff` を**足して**解決し、受入に **(3e)** を新設した。⛔ 弱めていない。

**4. ⚠⚠⚠ `world.html` の禁止語の罠を、禁止事項を説明するコメントの中で踏んだ**
実装窓は §2-6 を読んだうえで、コメントに `localStorage.setItem` と書いてしまい、
一時的に `localStorage.setItem` 1 件 / `localStorage.removeItem` 1 件 / `sessionStorage.removeItem` **2 件**に
なった(= `verify_road_events` (2c) が赤になる状態)。自力で言い換えて **0 / 0 / 1** へ戻した。
⭐ **一般解 = 禁止された語は「禁止だと書くコメント」の中でも禁止。実装直後に必ず数える。**

**5. ⭐ `applyQuestSubline("title")` は `consumeResult()` より後でなければならない**
依頼書は world についてしか順序を書いていなかった。酒場は `consumeResult` が器を消すので、
前に呼ぶと**帰還した回だけ依頼名が 1 回残る**。⇒ スクリプト最終行に置いた。

**6. ⭐ §9 (0c)「素の `world.html` で `.worldSign` が 7 枚」は、前提を仕込まないと永久に赤**
素では `UNLOCK` の鎖で**廃坑 + 港町の 2 枚**しか出ない。
`dragonfighters.cleared` に **5 件**仕込んで初めて 7 枚になる。

**7. ⭐ `world.html` に `--wood-mid` / `--wood-dark` が無い**(`--parchment-gold: #8b6914` だけ)
依頼書の CSS はフォールバック付きなので実害は無いが、**世界地図と街で戻るボタンの木目色が違う**
(街 `#5a3a1e` / `#3a2614`、世界地図 `#6b5334` / `#3f2f1c`)。⇒ 実機の目視で判断(13-7 の 6)。

**8. ⭐⭐⭐ `verify_npc_crowd` (3c) の赤は #64 固有ではなく、3 件目の顕在化だった**
押し所を「NPC・吹き出し・札」という**名指しの黒リスト**で除外し、
**主人公から最も遠いタイル**を選ぶ作りだったので、**画面に固定した UI が隅に 1 つ増えるたびに吸い寄せられる**。
実測で酒場でも漏れていた —— 酒場/desktop は `#changelogBox` の下、
酒場/compact は `#promisesBadge`(#60)の下を素通ししていた。
**最遠タイルにならなかったので赤くならなかっただけ**で、欠陥は #60 の時点で既に在った。
⇒ **白リスト**(「その点の `elementFromPoint` が舞台 `#tavernStage` / `#townStage` そのものか子孫」)へ言い直し、
装置 assert **(3c-pick)** を新設(32 → **33**)。
⭐ **一般解 = 「押し所を選ぶ」装置は、除外を名前で数え上げた瞬間に UI が 1 つ増えるたび腐る。
「押した先が目的の面に届いているか」で書けば腐らない。**
空振りしていないことは**合成の障害物を自分で被せて**示せる(実在の UI の位置に依存させない)。

**9. ⚠⚠ (3c) の腕 ③ には巡回 NPC 由来の間欠赤も潜んでいた**
街/desktop の最遠候補 `[19,11]` は strollB `(16,11)⇄(19,11)` の**折返し点そのもの**で、
pick と click の間に観測を 3 つ挟んだだけで **3 回に 1 回**落ちた。
対策 = 装置の測定を click の**後ろ**へ移す + **巡回経路の全マス ±1** を押し所から除外。
⛔ **pick と click の間には今後も何も挟まない。**

---

### 13-7. 残 = 実機/実感の確認(**7 件**)

§10 の 5 件に、着地後に判明した 2 件(3・4)を足したもの。

1. 封蝋の脈動が **390px の iPhone** でうるさくないか(速すぎたら周期を 2s から伸ばす)
2. 歩行中(街道を辿っている最中)に**駒が跳ねない**か
3. ⭐ 副行の**折り返しはヘッドレスで先に潰せた** —— 最長名
   (`受注中 — ドラゴンの巣「レッドドラゴン「ファラクサス」」`)でも compact 390 で帯の高さは
   **63px で不変**。残るのは「**16px で読めるか**」の目視だけ
4. ⚠⚠ **スマートフォン幅(390x844)の街で `#townBack` が歩ける地図を覆っている。**
   その下に入る歩けるタイルは **2 マス**(`[5,0]` を含む)。
   **街/desktop(1440x900)では 0 マス**なので **compact だけの問題**。
   位置を変えるなら「compact で row 0 の歩けるマスに掛からない」= 上端から 1 タイル分(**約 48px**)下へ、
   または右上へ寄せるのが最小の変更。
   ⚠ 動かしたら `verify_npc_crowd` の **(3c-pick) の腕 ② を腕 ① だけへ言い直す**こと(① は位置に依存しない)
5. 戻るボタンが**左上で親指に届く**か(右手持ちだと遠い可能性。右上へ移す判断は実機で)
6. ⭐ **世界地図と街で戻るボタンの木目色が違う**(13-6 の 7)。揃えるかどうか
7. 戻るボタンが**世界地図の札**と重なっていないか
   (街の看板は compact で 4 枚中 1 枚しか画面内に無いので別途)

---

### 13-8. changelog

`f5091a7` で追記済(`tavern.html:3267-3270` の 4 件)。先頭から:

1. `<b>はじめの画面へ戻れるようになった</b> — 街と世界地図の左上のボタンから、記録の選び直しができる。進行は消えない。`
2. `<b>依頼の地に赤い封蝋の印が付く</b> — 世界地図で向かうべき場所がひと目で分かる。`
3. `<b>受注中の依頼が画面にずっと出るようになった</b> — 地図と街と酒場の見出しの下に、いま受けている依頼の名前が表示される。`
4. `<b>廃墟の砦が卓上マップ 2 枚の戦場に</b> — …`(#63 由来。既定 4 件維持で残っている)

⛔ 項目4 は `.md` 2 枚しか触らないのでフックの対象外 = **足さない**。

---

### 13-9. ⚠ 測定環境の注意(次のチケットへの申し送り)

`verify_npc_crowd --negative` を **Bash ツール経由で走らせると、変異 4〜6 本目のあたりで
`exit 127` で無言で落ちる**(落ちる変異は毎回ずれるので、ドライバ側の欠陥ではない)。
落ちると **node が port 9573 を掴んだまま残り、次の起動が `EADDRINUSE` で即死する**。
⇒ **`PowerShell` ツールから同じコマンドを走らせると 58/58 exit 0 で完走する。**
⭐ 再走の前に、`verify_npc_crowd.js` の残骸プロセスと `df_npc_*` プロファイルの Chrome を
片付けてから始めること(残っていると必ず `EADDRINUSE`)。
